...
class TransferSettlementApplyRepository extends HrisRepository implements RepositoryInterface {
    ...
    public function getFilteredRecords(array $search) {
        ...
        if($search['startDate']){
            $condition = $condition. " and to_char(JH.Start_DATE,'DD-Mon-YYYY') = '{$search['startDate']}'";
        }

        if($search['endDate']){
            $condition = $condition. " and to_char(JH.END_DATE,'DD-Mon-YYYY') = ''{$search['endDate']}'";
        }

        if($search['eventDate']){
            $condition = $condition. " and to_char(JH.EVENT_DATE,'DD-Mon-YYYY') = '{$search['eventDate']}'";
        }
        $sql= " select * from (
            SELECT
           ...
           then case when ((select
                sum(family_no_travlled_with) 
                   from ( select
                distinct job_history_id,
                serial_number,
                family_no_travlled_with 
                       from HRIS_TRANSFER_SETTLEMENT 
                       where job_history_id = JH.job_history_id 
                       and status not in ('C',
                'R') ) 
                   group by job_history_id) < 3) 
           then 'Y' 
           else 'N' 
           end 
           else 'Y' 
