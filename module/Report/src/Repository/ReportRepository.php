...
//        return $result;
    ...
    private function convertLeaveIdToName($allLeave, $leaveData, $name) {
        ...
        foreach ($leaveData as $report) {
            ...
            foreach ($allLeave as $leave) {
                $tempData[$leave['LEAVE_TRIM_ENAME']] = $report['L_'.$leave['LEAVE_ID']];
            }
    ...
    public function filterLeaveReportEmployee($data) {
        ...
        $leaveIn = explode(',', $this->leaveIn($allLeave));
        ...
        foreach ($leaveIn as $pivotValue) {
          $startSql .= " , SUM(case when leave_id = {$pivotValue} then leave_days end) as L_{$pivotValue}";
        }
    ...
    public function filterLeaveReportBranch($data) {
        ...
        $startSql = " SELECT BB.BRANCH_NAME,AA.* FROM (SELECT
                          *
                        FROM
                          (select branch_id ";
    ...
    public function filterLeaveReportDepartmnet($data) {
        ...
        $endSql = " FROM
                ...
                RIGHT JOIN HRIS_DEPARTMENTS BB
    ...
    public function filterLeaveReportDesignation($data) {
        ...
        $endSql = " FROM
                        ...
                        JOIN HRIS_EMPLOYEES E
                        ON (E.EMPLOYEE_ID=AD.EMPLOYEE_ID)
                        JOIN HRIS_DESIGNATIONS D
                        ON (D.DESIGNATION_ID=E.DESIGNATION_ID)
                        WHERE AD.LEAVE_ID  IS NOT NULL
                        GROUP BY D.DESIGNATION_ID,
                          AD.LEAVE_ID
                        )
                      group by DESIGNATION_ID) AA
                    RIGHT JOIN HRIS_DESIGNATIONS BB
    ...
    public function filterLeaveReportPosition($data) {
        ...
        $endSql = " FROM
                    ...
                    RIGHT JOIN HRIS_POSITIONS BB
                    ON (AA.POSITION_ID=BB.POSITION_ID
                    AND BB.STATUS     ='E') ";
    ...
    public function FetchNepaliMonth() {
        ...
        $select->where(["M.STATUS='E'", "FY.STATUS='E'", "CURRENT_DATE>M.FROM_DATE"]);
    ...
    public function branchWiseDailyReport($data) {
        ...
        $sql = <<<EOT
        SELECT 
                      days_between((M.FROM_DATE), (AD.ATTENDANCE_DT)) +1                              AS DAY_COUNT, 
                      E.EMPLOYEE_ID                                                             	AS EMPLOYEE_ID ,
                      E.EMPLOYEE_CODE                                                               AS EMPLOYEE_CODE,
                      ...
                      (
                      CASE
                        WHEN (AD.DAYOFF_FLAG ='N' OR (AD.DAYOFF_FLAG = 'Y' 
                        --AND C.HD_DO_CHECK = 1
                        ))
                        AND AD.LEAVE_ID   IS NULL
                        AND (AD.HOLIDAY_ID  IS NULL OR (AD.HOLIDAY_ID IS NOT NULL 
                        --AND C.HD_DO_CHECK = 1
                        ))
                        AND AD.TRAINING_ID IS NULL
                        AND AD.TRAVEL_ID   IS NULL
                    ...
                    and E.EMPLOYEE_ID not in (select employee_id from hris_job_history where RETIRED_FLAG = 'Y' or DISABLED_FLAG = 'Y')
                     AND (E.SERVICE_TYPE_ID IN (SELECT SERVICE_TYPE_ID FROM HRIS_SERVICE_TYPES WHERE TYPE NOT IN ('RESIGNED','RETIRED')) OR E.SERVICE_TYPE_ID IS NULL)
...
EOT;
    ...
    public function getDaysInMonth($monthId) {
        ...
        $sql = "SELECT DAYS_BETWEEN(FROM_DATE, TO_DATE) +1 AS TOTAL_DAYS FROM HRIS_MONTH_CODE WHERE MONTH_ID =?";
        $boundedParameter = [];
        $boundedParameter['monthId'] = $monthId;
        return $this->rawQuery($sql, $boundedParameter);
        // $statement = $this->adapter->query($sql);
    ...
    public function reportWithOT($data) {
        ...
        if (isset($data['fromDate']) && $data['fromDate'] != null && $data['fromDate'] != -1) {
            ...
            // $boundedParams['fromDate1'] = $fromDate;
            // $boundedParams['fromDate2'] = $fromDate;
            $fromCondition = "AND A.ATTENDANCE_DT >= to_date('{$fromDate}', 'DD-Mon-YYYY')";
            $otFromCondition = "AND OVERTIME_DATE >= to_date('{$fromDate}', 'DD-Mon-YYYY')";
        ...
        if (isset($data['toDate']) && $data['toDate'] != null && $data['toDate'] != -1) {
            ...
            $toCondition = "AND A.ATTENDANCE_DT <= to_date('{$toDate}', 'DD-Mon-YYYY')";
            $otToCondition = "AND OVERTIME_DATE <= to_date('{$toDate}', 'DD-Mon-YYYY')";
        ...
        $sql = <<<EOT
            SELECT C.COMPANY_NAME,
              ...
              A.UNPAID_LEAVE,
              A.ABSENT,
              IFNULL(ROUND(A.TOTAL_MIN/60,2),0) + IFNULL(AD.ADDITION,0) - IFNULL(AD.DEDUCTION,0) AS OVERTIME_HOUR,
              A.TRAVEL,
              A.TRAINING,
              A.WORK_ON_HOLIDAY,
              A.WORK_ON_DAYOFF,
...
GROUP BY
    ...
    overtime_date) OT
            ...
            ON (A.EMPLOYEE_ID = AD.EMPLOYEE_ID AND AD.MONTH_ID = ?)
...
EOT;
    ...
    public function departmentMonthReport($fiscalYearId) {
      ...
      $startSql = " SELECT D.DEPARTMENT_NAME,
                      R.*
                    FROM
                      (SELECT department_id ";
  ...
  $months[1] = "one";
          ...
          for($i = 1; $i <=12; $i++){
                $startSql .= "
                ,  MAX( CASE when FISCAL_YEAR_MONTH_NO = {$i} then PRESENT else 0 END ) as {$months[$i]}_PRESENT
                ,  MAX( CASE when FISCAL_YEAR_MONTH_NO = {$i} then ABSENT else 0 END ) as {$months[$i]}_ABSENT
                ,  MAX( CASE when FISCAL_YEAR_MONTH_NO = {$i} then LEAVE else 0 END ) as {$months[$i]}_LEAVE
                ,  MAX( CASE when FISCAL_YEAR_MONTH_NO = {$i} then DAYOFF else 0 END ) as {$months[$i]}_DAYOFF
                ,  MAX( CASE when FISCAL_YEAR_MONTH_NO = {$i} then HOLIDAY else 0 END ) as {$months[$i]}_HOLIDAY
                ,  MAX( CASE when FISCAL_YEAR_MONTH_NO = {$i} then WORK_ON_HOLIDAY else 0 END ) as {$months[$i]}_WOH
                ,  MAX( CASE when FISCAL_YEAR_MONTH_NO = {$i} then WORK_ON_DAYOFF else 0 END ) as {$months[$i]}_WOD ";
          }
    ...
    public function employeeMonthlyReport($searchQuery) {
        ...
        $startSql = " SELECT D.FULL_NAME, D.EMPLOYEE_CODE,
                      R.*
                    FROM
                      (SELECT employee_id ";
        ...
        $endSql = " FROM
                    ...
                    ON (A.LEAVE_ID          = L.LEAVE_ID)
                    WHERE EMC.FISCAL_YEAR_ID=?
                    GROUP BY  EMC.FISCAL_YEAR_MONTH_NO,EMC.EMPLOYEE_ID) GROUP by employee_Id
                  ) R
                left JOIN HRIS_EMPLOYEES D
...
EOT;
    ...
    public function employeeYearlyReport($employeeId, $fiscalYearId) {
        $startSql = "
                ...
                CL.WORK_HOLIDAY,
                 (CL.PRESENT+CL.ABSENT+CL.LEAVE+CL.DAYOFF+CL.HOLIDAY+CL.WORK_DAYOFF+CL.WORK_HOLIDAY) as TOTAL
           FROM  (SELECT full_name, EMPLOYEE_ID, month_id, MONTH_DTL, FISCAL_YEAR_MONTH_NO";
      ...
      days_between(AD.ATTENDANCE_DT,MC.FROM_DATE)+1 AS DAY_COUNT
      FROM HRIS_ATTENDANCE_DETAIL AD
      LEFT JOIN HRIS_LEAVE_MASTER_SETUP LMS ON (AD.LEAVE_ID = LMS.LEAVE_ID)
      ...
      WHERE AD.EMPLOYEE_ID = {$employeeId})
      group by full_name, EMPLOYEE_ID, month_id, MONTH_DTL, FISCAL_YEAR_MONTH_NO
      ...
      for($i = 1; $i <=32; $i++){
        $startSql .= "
        , MAX(case when DAY_COUNT = {$i} then OVERALL_STATUS end) as D{$days[$i]}";
      }
    ...
    public function fetchBirthdays($by) {
        ...
        $sql = "SELECT  
          {$columIfSynergy}
              ...
              {$joinIfSyngery}
              WHERE 1=1 AND 
              to_number(to_char(TO_DATE(E.BIRTH_DATE, 'YYYY-MM-DD'), 'MMDD')) 
              BETWEEN to_number(to_char(TO_DATE('{$fromDate}', 'DD-MON-YYYY') , 'MMDD'))
              AND to_number(to_char(TO_DATE('{$toDate}', 'DD-MON-YYYY') , 'MMDD'))
              AND E.STATUS='E' 
              {$condition['sql']}
              {$orderByString}";
        return $this->rawQuery($sql, $boundedParameter);
...
$sql = "SELECT E.EMPLOYEE_CODE, E.FULL_NAME, E.JOIN_DATE DOJ, E.BIRTH_DATE DOB,
P.Position_Name, e.salary, e.allowance, (ifnull(e.salary, 0)+ifnull(e.allowance, 0)) gross,
    Des.Designation_Title,
    D.Department_Name,
    Funt.Functional_Type_Edesc,        
     St.Service_Type_Name,
     FLOOR(DAYS_BETWEEN(BIRTH_DATE,CURRENT_DATE)/365)||' Years '||FLOOR((DAYS_BETWEEN(BIRTH_DATE,CURRENT_DATE)/365-FLOOR(DAYS_BETWEEN(BIRTH_DATE,CURRENT_DATE)/365))*365)||' Days' AGE ,
     FLOOR(DAYS_BETWEEN(JOIN_DATE,CURRENT_DATE)/365)||' Years '||FLOOR((DAYS_BETWEEN(JOIN_DATE,CURRENT_DATE)/365-FLOOR(DAYS_BETWEEN(JOIN_DATE,CURRENT_DATE)/365))*365)||' Days' SERVICE_DURATION
    FROM HRIS_EMPLOYEES E 
    LEFT JOIN HRIS_DESIGNATIONS DES
      ON E.DESIGNATION_ID=DES.DESIGNATION_ID 
      LEFT JOIN HRIS_POSITIONS P
        ...
        $sql = <<<EOT
              ...
              FROM
                (SELECT EMPLOYEE_CODE,
                  ...
                  FLOOR(months_between(BIRTH_DATE,CURRENT_DATE)/12) AS AGE,
                  STATUS
                FROM HRIS_EMPLOYEES
                )E
              LEFT JOIN HRIS_DEPARTMENTS D
...
EOT;
    ...
    public function checkRetirementEmployee($data) {
      $greaterCondition = "";
      $lessCondition = "";
      $bothCondition = "";
      ...
      /* for days */
      if ($data['greaterThan'] != null && $data['lessThan'] == null) {
          //$greaterCondition = "AND RDAYS >= :greaterThan";
          $greaterCondition = "AND RDAYS >=". $data['greaterThan'] . " AND RYEARS=0 AND RMONTHS=0";
          $boundedParameter['greaterThan'] = $data['greaterThan'];
      }
      if ($data['greaterThan'] == null && $data['lessThan'] != null) {
          //$lessCondition = "AND RDAYS <= :lessThan ";
          $lessCondition = "AND RDAYS <= ". $data['lessThan'] . " AND RYEARS=0 AND RMONTHS=0";
          $boundedParameter['lessThan'] = $data['lessThan'];
      }
      if ($data['greaterThan'] != null && $data['lessThan'] != null) {
          //$bothCondition = "AND RDAYS between :greaterThan and :lessThan ";
          $bothCondition = "AND RDAYS between " . $data['greaterThan']. " and " . $data['lessThan'] ." AND RYEARS=0 AND RMONTHS=0" ;
          $boundedParameter['greaterThan'] = $data['greaterThan'];
          $boundedParameter['lessThan'] = $data['lessThan'];
      }

      /* For Month */
      if ($data['greaterThanM'] != null && $data['lessThanM'] == null) {
        //$greaterCondition = "AND RMONTHS >= :greaterThanM";
        $greaterCondition = "AND RMONTHS >=". $data['greaterThanM']." AND RYEARS=0";
        $boundedParameter['greaterThanM'] = $data['greaterThanM'];
      }
      if ($data['greaterThanM'] == null && $data['lessThanM'] != null) {
          //$lessCondition = "AND RMONTHS <= :lessThanM ";
          $lessCondition = "AND RMONTHS <= ". $data['lessThanM']." AND RYEARS=0";
          $boundedParameter['lessThanM'] = $data['lessThanM'];
      }
      if ($data['greaterThanM'] != null && $data['lessThanM'] != null) {
          //$bothCondition = "AND RMONTHS between :greaterThanM and :lessThanM ";
          $bothCondition = "AND RMONTHS between " . $data['greaterThanM']. " and " . $data['lessThanM'] ." AND RYEARS=0";
          $boundedParameter['greaterThanM'] = $data['greaterThanM'];
          $boundedParameter['lessThanM'] = $data['lessThanM'];
      }

    /* For Year */
    if ($data['greaterThanY'] != null && $data['lessThanY'] == null) {
      //$greaterCondition = "AND RYEARS >= :greaterThanY";
      $greaterCondition = "AND RYEARS >=". $data['greaterThanY'];
      $boundedParameter['greaterThanY'] = $data['greaterThanY'];
    }
    if ($data['greaterThanY'] == null && $data['lessThanY'] != null) {
        //$lessCondition = "AND RYEARS <= :lessThanY ";
        $lessCondition = "AND RYEARS <= ". $data['lessThanY'];
        $boundedParameter['lessThanY'] = $data['lessThanY'];
    }
    if ($data['greaterThanY'] != null && $data['lessThanY'] != null) {
        //$bothCondition = "AND RYEARS between :greaterThanY and :lessThanY ";
        $bothCondition = "AND RYEARS between " . $data['greaterThanY']. " and " . $data['lessThanY'] ;
        $boundedParameter['greaterThanY'] = $data['greaterThanY'];
        $boundedParameter['lessThanY'] = $data['lessThanY'];
    }
      ...
      $sql = <<<EOT
      SELECT * FROM(
        SELECT  employee_id, employee_code, full_name,bs_date(join_date) as join_date, bs_date(add_years(birth_date,58)) est_retirement_date,   FLOOR((MONTHS_BETWEEN ( CURRENT_DATE,add_years(birth_date,58)) / 12))
             || ' Years '
             || MOD (floor(MONTHS_BETWEEN ( CURRENT_DATE,add_years(birth_date,58))), 12)
             || ' Months '
             || (DAYS_BETWEEN( ADD_MONTHS (CURRENT_DATE,FLOOR (MONTHS_BETWEEN (CURRENT_DATE,add_years(birth_date,58)))),TO_DATE (add_years(birth_date,58))))
             || ' Days' as est_retirement_remaining_days    ,
              FLOOR((MONTHS_BETWEEN ( CURRENT_DATE,add_years(birth_date,58)) / 12))
             as ryears,
              MOD (floor(MONTHS_BETWEEN ( CURRENT_DATE,add_years(birth_date,58))), 12) as rMonths,
             (DAYS_BETWEEN( ADD_MONTHS (CURRENT_DATE,FLOOR (MONTHS_BETWEEN (CURRENT_DATE,add_years(birth_date,58)))),TO_DATE (add_years(birth_date,58))))
              as  rDays
        FROM HRIS_EMPLOYEES) WHERE est_retirement_date is not null AND RMONTHS>=0 AND RYEARS>=0 {$greaterCondition} {$lessCondition} {$bothCondition}
EOT;
//echo 'sql='.$sql;die;
      return $this->rawQuery($sql, $boundedParameter);
      // $statement = $this->adapter->query($sql);
    ...
    public function checkContract($searchQuery) {
        ...
        $sql = "SELECT S.*,
                ...
                B.BRANCH_NAME,
                Case WHEN
                (S.END_DATE >= current_date OR S.END_DATE IS NULL)
                THEN 'Not Expired'
                WHEN
              ...
              FROM
                ...
                FROM
                  (SELECT JH.EMPLOYEE_ID,
                    ...
                    JH.END_DATE,
                    TYPE,
                    round(months_between(END_DATE,current_date)) AS REMAINING_MONTHS
...
//             PL.EMPLOYEE_CODE, ';
        ...
        $boundedParameter['fromDate1'] = $data['fromDate'];
        $boundedParameter['fromDate2'] = $data['fromDate'];
        $boundedParameter['fromDate3'] = $data['fromDate'];
        $boundedParameter['fromDate4'] = $data['fromDate'];
        $boundedParameter['fromDate5'] = $data['fromDate'];
        $boundedParameter['fromDate6'] = $data['fromDate'];

        $boundedParameter['toDate1'] = $data['toDate'];
        $boundedParameter['toDate2'] = $data['toDate'];
        $boundedParameter['toDate3'] = $data['toDate'];
        $boundedParameter['toDate4'] = $data['toDate'];
        $boundedParameter['toDate5'] = $data['toDate'];
        $boundedParameter['toDate6'] = $data['toDate'];
...
,E.FULL_NAME,
      ...
      CASE
        ...
        AND AD.ATTENDANCE_DT    > CURRENT_DATE
        THEN 'O'
        ELSE AD.OVERALL_STATUS
      END                                                         AS OVERALL_STATUS,
      (AD.ATTENDANCE_DT- ? +1) AS DAY_COUNT_ATTD
      ,AD.ATTENDANCE_DT
...
from
(SELECT TO_DATE(?, 'DD-MON-YY') + RowNum - 1 AS DATES,
  RowNum                                            AS DAY_COUNT,
  ?               AS FROM_DATE
FROM dual d
  CONNECT BY RowNum <= TO_DATE(?, 'DD-MON-YY')
  - TO_DATE(?, 'DD-MON-YY') + 1 ) AD
...
LEFT JOIN
  (SELECT EMPLOYEE_ID,
    ...
    END) AS TRAVEL
  FROM HRIS_ATTENDANCE_DETAIL
  WHERE ATTENDANCE_DT BETWEEN to_date(?,'DD-Mon-YYYY') AND to_date(?,'DD-Mon-YYYY')
  GROUP BY EMPLOYEE_ID
  )CL
ON (PL.EMPLOYEE_ID=CL.EMPLOYEE_ID)
LEFT JOIN
  ...
  FROM
    ...
    AND AD.ATTENDANCE_DT BETWEEN to_date(?,'DD-Mon-YYYY') AND to_date(?,'DD-Mon-YYYY')
...
EOT;
    ...
    public function getDates($monthId) {
        $boundedParam = [];
        $sql = "SELECT TO_DATE, FROM_DATE, MONTH_EDESC FROM HRIS_MONTH_CODE WHERE MONTH_ID = ?";
   ...
   public function getMonthDetailsForKendo($fromDate,$toDate){
    ...
    public function getMonthDetailsByDate($fromDate,$toDate) {
        $boundedParam = [];
        $boundedParam['fromDate1']=$fromDate;
        $boundedParam['toDate1']=$toDate;
        $boundedParam['fromDate2']=$fromDate;
        $boundedParam['toDate2']=$toDate;

         $sql = "SELECT 
         TO_DATE(?,'DD-MON-YYYY') AS FROM_DATE,
         TO_DATE(?,'DD-MON-YYYY') AS TO_DATE,
         DAYS_BETWEEN(TO_DATE(?,'DD-MON-YYYY'),TO_DATE(?,'DD-MON-YYYY')) AS DAYS FROM
         DUMMY";
...
FROM
    ...
    public function filterAttendanceRecord($companyId, $branchId, $departmentId, $designationId, $positionId, $serviceTypeId, $serviceEventTypeId, $employeeTypeId, $genderId, $functionalTypeId, $employeeId, $fromDate = null, $toDate = null, $status = null, $presentStatus = null, $min = null, $max = null, $presentType = null, $locationId, $monthId) {
      $boundedParams = [];
      $searchConditon = EntityHelper::getSearchConditonBounded($companyId, $branchId, $departmentId, $positionId, $designationId, $serviceTypeId, $serviceEventTypeId, $employeeTypeId, $employeeId, $genderId,$locationId, $functionalTypeId);
      $boundedParams = array_merge($boundedParams, $searchConditon['parameter']);


      $sql = "select E.Employee_code, E.full_name,hd.designation_title, hl.location_edesc,hf.functional_level_edesc,
      had.attendance_dt, hs.shift_id,
      case
      when (had.overall_status = 'WH' or had.overall_status = 'WD') then 'H'
      when (WEEKDAY (TO_DATE (had.attendance_dt, 'YYYY-MM-DD'))) = 4 then 'F'
      else 'R' end as day_code, BS_DATE(had.attendance_dt) as ATTENDANCE_DATE_BS,to_char(had.in_time,'HH:MI:SS') as IN_TIME, 
            to_char(had.out_time,'HH:MI:SS') as out_time,
            seconds_between(had.in_time,had.out_time)/3600 total_hour,
      case 
        when (had.overall_status = 'WH' or had.overall_status = 'WD')
        then (seconds_between(had.in_time,had.out_time)/60)/60
        when (WEEKDAY (TO_DATE (had.attendance_dt, 'YYYY-MM-DD'))) = 4 
        then  (seconds_between(had.in_time,had.out_time)/60 - 300)/60
        else 
        (seconds_between(had.in_time,had.out_time)/60 - hs.total_working_hr)/60 
        end as OT_HOUR 
            from hris_attendance_detail had join hris_employees E on (E.employee_id=had.employee_id)
            join hris_designations hd on (hd.designation_id=E.designation_id)
            join hris_locations hl on (hl.location_id=E.location_id)
            join hris_functional_levels hf on (hf.functional_level_id=E.functional_level_id)
            join hris_shifts hs on (hs.shift_id = had.shift_id)
            
            where had.attendance_dt between 
            (select from_date from hris_month_code where month_id = {$monthId}) and 
            (select to_date from hris_month_code where month_id = {$monthId}) and E.status='E'
  
           AND (E.SERVICE_TYPE_ID IN (SELECT SERVICE_TYPE_ID FROM HRIS_SERVICE_TYPES WHERE TYPE NOT IN ('RESIGNED','RETIRED')) OR E.SERVICE_TYPE_ID IS NULL)
          order by E.full_name, had.attendance_dt asc"
              ;
  ...
  public function organogramReport(){
    $sql = "SELECT department_id, department_name,parent_department from hris_departments";
    $statement = $this->adapter->query($sql);
    $result = $statement->execute();
    return Helper::extractDbData($result);
  }
  ...
  public function departmentDetails($id){
    ...
    $sql = "SELECT  * from hris_departments where department_id = {$id}";
    $statement = $this->adapter->query($sql);
    $result = $statement->execute();
    return Helper::extractDbData($result);
  }
  ...
  public function branchDetails($id){
    $sql = "SELECT  * from hris_branches_an where branch_id = {$id}";
    $statement = $this->adapter->query($sql);
    $result = $statement->execute();
    return Helper::extractDbData($result);
  }

  public function getChildDepartment($depId){
    $sql = "SELECT * FROM HRIS_DEPARTMENTS 
    WHERE parent_department = {$depId} and status = 'E' and (department_code not in ('NA', 'DEFAULT') or department_code is null)
    and department_id not in (select department_id from HRIS_ORGANOGRAM_SETUP where status = 'E')
    and department_id not in (select twins_department from hris_organogram_setup where status='E'
    and twins_department is not null and has_twins = 'Y')";
    $statement = $this->adapter->query($sql);
    $result = $statement->execute();
    return Helper::extractDbData($result);
  }

  public function getChildBranch($depId){
    $sql = "SELECT * FROM HRIS_BRANCHES 
    WHERE parent_department in ({$depId}) and status = 'E'";
    $statement = $this->adapter->query($sql);
    $result = $statement->execute();
    return Helper::extractDbData($result);
  }

  public function getChildLocation($branchId){
    $sql = "SELECT * FROM HRIS_LOCATIONS  WHERE branch_id = {$branchId} and status = 'E'";
    $statement = $this->adapter->query($sql);
    $result = $statement->execute();
    return Helper::extractDbData($result);
  }
