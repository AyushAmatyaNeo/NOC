...
class WorkOnDayoffStatusRepository extends HrisRepository {
    ...
    public function getFilteredRecord($data, $recomApproveId) {
        ...
        if ($fromDate != null) {
            $fromDateCondition = " AND WD.FROM_DATE>=TO_DATE($fromDate,'YYYY-MM-DD')";
            $boundedParameter['fromDate'] = $fromDate;
        }

        if ($toDate != null) {
            $toDateCondition = "AND WD.TO_DATE<=TO_DATE($toDate,'YYYY-MM-DD')";
            $boundedParameter['toDate'] = $toDate;
        }

        $sql = "SELECT INITCAP(TO_CHAR(WD.FROM_DATE, 'YYYY-MM-DD'))              AS FROM_DATE_AD,
                  BS_DATE(TO_CHAR(WD.FROM_DATE, 'YYYY-MM-DD'))                   AS FROM_DATE_BS,
                  INITCAP(TO_CHAR(WD.TO_DATE, 'YYYY-MM-DD'))                     AS TO_DATE_AD,
                  BS_DATE(TO_CHAR(WD.TO_DATE, 'YYYY-MM-DD'))                     AS TO_DATE_BS,
                  INITCAP(TO_CHAR(WD.REQUESTED_DATE, 'YYYY-MM-DD'))              AS REQUESTED_DATE_AD,
                  BS_DATE(TO_CHAR(WD.REQUESTED_DATE, 'YYYY-MM-DD'))              AS REQUESTED_DATE_BS,
...
//        }
         $sql = "SELECT INITCAP(TO_CHAR(WD.FROM_DATE, 'YYYY-MM-DD'))              AS FROM_DATE_AD,
                   BS_DATE((WD.FROM_DATE))                   AS FROM_DATE_BS,
                   INITCAP(TO_CHAR(WD.TO_DATE, 'YYYY-MM-DD'))                     AS TO_DATE_AD,
                   BS_DATE((WD.TO_DATE))                     AS TO_DATE_BS,
                   INITCAP(TO_CHAR(WD.REQUESTED_DATE, 'YYYY-MM-DD'))              AS REQUESTED_DATE_AD,
                   BS_DATE((WD.REQUESTED_DATE))              AS REQUESTED_DATE_BS,
                   LEAVE_STATUS_DESC(WD.STATUS)                                    AS STATUS,
                   WD.REMARKS                                                      AS REMARKS,
                   WD.DURATION                                                     AS DURATION,
                   WD.EMPLOYEE_ID                                                  AS EMPLOYEE_ID,
                   WD.ID                                                           AS ID,
                   WD.MODIFIED_DATE                                                AS MODIFIED_DATE,
                   INITCAP(TO_CHAR(WD.RECOMMENDED_DATE, 'YYYY-MM-DD'))            AS RECOMMENDED_DATE,
                   INITCAP(TO_CHAR(WD.APPROVED_DATE, 'YYYY-MM-DD'))               AS APPROVED_DATE,
                   E.EMPLOYEE_CODE                                                 AS EMPLOYEE_CODE,
                   INITCAP(E.FULL_NAME)                                            AS FULL_NAME,
                   INITCAP(E1.FULL_NAME)                                           AS RECOMMENDED_BY_NAME,
                   INITCAP(E2.FULL_NAME)                                           AS APPROVED_BY_NAME,
        ...
        if ($fromDate != null) {
          $boundedParameter['fromDate'] = $fromDate;  
          $sql .= " AND WD.FROM_DATE>=TO_DATE(?,'DD-MM-YYYY')";
        }

        if ($toDate != null) {
          $boundedParameter['toDate'] = $toDate;
          $sql .= "AND WD.TO_DATE<=TO_DATE(?,'DD-MM-YYYY')";
        }
