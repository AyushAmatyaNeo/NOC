...
class DayoffWorkApproveRepository implements RepositoryInterface {
    ...
    public function edit(Model $model, $id) {
      ...
      BEGIN
        ...
        WHERE ID                                    = {$id};
        IF(V_STATUS IN ('AP','C','R') and V_FROM_DATE <= current_date) THEN
...
END;
    ...
    public function getAllRequest($id, $status = null) {
        $sql = "SELECT 
                    ...
                    WD.EMPLOYEE_ID,
                    INITCAP(TO_CHAR(WD.REQUESTED_DATE, 'YYYY-MM-DD')) AS REQUESTED_DATE,
                    BS_DATE(TO_CHAR(WD.REQUESTED_DATE, 'YYYY-MM-DD')) AS REQUESTED_DATE_N,
                    WD.APPROVED_BY,
                    WD.RECOMMENDED_BY,
                    WD.REMARKS,
                    WD.DURATION,
                    ...
                    WD.APPROVED_REMARKS,
                    INITCAP(TO_CHAR(WD.FROM_DATE, 'YYYY-MM-DD')) AS FROM_DATE,
                    BS_DATE(TO_CHAR(WD.FROM_DATE, 'YYYY-MM-DD')) AS FROM_DATE_N,
                    INITCAP(TO_CHAR(WD.TO_DATE, 'YYYY-MM-DD')) AS TO_DATE,
                    BS_DATE(TO_CHAR(WD.TO_DATE, 'YYYY-MM-DD')) AS TO_DATE_N,
                    ...
                    INITCAP(TO_CHAR(WD.MODIFIED_DATE, 'YYYY-MM-DD')) AS MODIFIED_DATE,
