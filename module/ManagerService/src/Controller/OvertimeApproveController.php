...
class OvertimeApproveController extends HrisController {
    ...
    public function viewAction() {
        ...
        if ($request->isPost()) {
            ...
            $this->makeDecisionApprovedReject($id, $role, $action == 'Approve', $action == 'Reject', $postedData[$role == 2 ? 'recommendedRemarks' : 'approvedRemarks'], true);
            return $this->redirect()->toRoute("overtimeApprove");
        }
        $overtimeModel->exchangeArrayFromDB($detail);
        $overtimeModel->description = $detail['IN_DESCRIPTION'];
    ...
    private function makeDecision($id, $role, $approve, $remarks = null, $enableFlashNotification = false) {
     ...
     private function makeDecisionApprovedReject($id, $role, $approve, $reject, $remarks = null, $enableFlashNotification = false) {
        $notificationEvent = null;
        $message = null;
        $model = new Overtime();
        $model->overtimeId = $id;
