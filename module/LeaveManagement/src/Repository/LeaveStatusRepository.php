...
class LeaveStatusRepository extends HrisRepository {
    ...
    public function getAllRequest($status = null, $date = null, $branchId = NULL, $employeeId = NULL) {
        ...
        if ($branchId != null) {
            $sql .= " AND E." . HrEmployees::EMPLOYEE_ID . " IN (SELECT " . HrEmployees::EMPLOYEE_ID . " FROM " . HrEmployees::TABLE_NAME . " WHERE " . HrEmployees::BRANCH_ID . "= ?)";
            $boundedParameter['branchId'] = $branchId;
        }

        if ($employeeId != null) {
            $sql .= "AND E." . HrEmployees::EMPLOYEE_ID . " = ?";
    ...
    public function getFilteredRecord($data, $recomApproveId = null) {
        ...
        if ($leaveRequestStatusId != -1) {
            $statusCondition = " AND LA.STATUS= ?";
            $boundedParameter['leaveRequestStatusId'] = $leaveRequestStatusId;
        }

        if ($leaveId != null && $leaveId != -1) {
            $leaveCondition = " AND LA.LEAVE_ID = ?";
            $boundedParameter['leaveId'] = $leaveId;
        }

        if ($fromDate != null) {
            $fromDateCondition = " AND LA.START_DATE>=TO_DATE('{$fromDate}','DD-MON-YYYY')";
        }

        if ($toDate != null) {
            $toDateCondition = "AND LA.END_DATE<=TO_DATE('{$toDate}','DD-MON-YYYY')";
        }
    ...
    public function getLeaveRequestList($data): array {
        ...
        if ($fromDate != null) {
            $fromDateCondition = " AND LA.START_DATE>=TO_DATE(?,'DD-MON-YYYY')";
            $boundedParameter['fromDate'] = $fromDate;
        }

        if ($toDate != null) {
            $toDateCondition = "AND LA.END_DATE<=TO_DATE(?,'DD-MON-YYYY')";
            $boundedParameter['toDate'] = $toDate;
        }
        
        if($toDate != null && $fromDate != null){
            $fromDateCondition="";
            $toDateCondition="AND ( ( LA.START_DATE BETWEEN TO_DATE (?,'DD-MON-YYYY') AND TO_DATE(?,'DD-MON-YYYY') )
                                OR  ( LA.END_DATE   BETWEEN TO_DATE (?,'DD-MON-YYYY') AND TO_DATE(?,'DD-MON-YYYY') ) )";
            $boundedParameter['fromDate1'] = $fromDate;
            $boundedParameter['toDate1'] = $toDate;
            $boundedParameter['fromDate2'] = $fromDate;
            $boundedParameter['toDate2'] = $toDate;
        }
        ...
        $sql = "SELECT 
                ...
                AS LEAVE_ENAME,
                  ...
                  INITCAP(TO_CHAR(LA.REQUESTED_DT, 'DD-MON-YYYY'))   AS APPLIED_DATE_AD,
                  BS_DATE((LA.REQUESTED_DT))   AS APPLIED_DATE_BS,
                  (select LEAVE_STATUS_DESC(LA.STATUS) from DUMMY)   AS STATUS,
                  LA.ID                                              AS ID,
                  LA.EMPLOYEE_ID                                     AS EMPLOYEE_ID,
                  INITCAP(TO_CHAR(LA.RECOMMENDED_DT, 'DD-MON-YYYY')) AS RECOMMENDED_DT,
                  INITCAP(TO_CHAR(LA.APPROVED_DT, 'DD-MON-YYYY'))    AS APPROVED_DT,
  ...
  AND EMPLOYEE_ID = ? ";
    ...
    public function delete($id) {
        ...
        $sql="update HRIS_EMPLOYEE_LEAVE_REQUEST set status='R' where ID = $id";
      $this->rawQuery($sql);
            ...
            DO
                    ...
                    BEGIN
                      ...
                      IF(V_STATUS IN ('AP','C', 'R')) THEN
                        CALL HRIS_REATTENDANCE(V_START_DATE,V_EMPLOYEE_ID,V_END_DATE);
						CALL HRIS_RECALCULATE_LEAVE (V_EMPLOYEE_ID,V_LEAVE_ID);
                      END IF;
                    END;
                END;
    ");
