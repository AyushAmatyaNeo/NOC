...
class LeaveAssignRepository extends HrisRepository {
    ...
    public function filter($branchId, $departmentId, $genderId, $designationId, $serviceTypeId, $employeeId, $companyId, $positionId, $employeeTypeId, $leaveId): array {
        ...
        $sql = "SELECT C.COMPANY_NAME,
                ...
                LEFT JOIN (SELECT * FROM HRIS_EMPLOYEE_LEAVE_ASSIGN WHERE LEAVE_ID   = ?) ELA
    ...
    public function filterByLeaveEmployeeId($leaveId, $employeeId, $fiscalYearMonthNo = null) {
        $result = $fiscalYearMonthNo == null ? $this->tableGateway->select([LeaveAssign::LEAVE_ID => $leaveId, LeaveAssign::EMPLOYEE_ID => $employeeId]) : $this->tableGateway->select([LeaveAssign::LEAVE_ID => $leaveId, LeaveAssign::EMPLOYEE_ID => $employeeId, LeaveAssign::FISCAL_YEAR_MONTH_NO => $fiscalYearMonthNo]);
    ...
    //     $sql="DECLARE
    //         V_DEFAULT_LEAVE_DAYS NUMBER:= ?;
    //         V_LEAVE_ID NUMBER:= ?;
    //         V_MONTH_ID NUMBER:= ?;
    //  V_COUNT NUMBER;
    //  V_EMPLOYEE_ID NUMBER:= ?;
    //      V_MONTH_COUNT NUMBER:=1;
    //      V_CARRY_FORWARD CHAR(1 BYTE):= ?;
    //      V_PREVIOUS_YEAR_BAL NUMBER:= ?;
    // CALL
    ...
    //     ";
            $sql = "CALL HRIS_ASSIGN_MONTHLY_LEAVE(
                $monthlyDays,
                {$leaveDetails['LEAVE_ID']},
                $monthId,
                $employeeId,
                '{$leaveDetails['CARRY_FORWARD']}',
                $previousBalance
