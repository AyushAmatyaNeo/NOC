...
use LeaveManagement\Model\LeaveEncashmenthr;
use Zend\Db\Adapter\AdapterInterface;
use Zend\Db\TableGateway\TableGateway;

class LeaveEncashmenthrRepository extends HrisRepository {
    ...
    public function __construct(AdapterInterface $adapter) {
        $this->tableGateway = new TableGateway(LeaveEncashmenthr::TABLE_NAME, $adapter);
    ...
    public function  getLeaveEncashment($employeeId,$fiscalId){

        // print_r($fiscalId);die;
        $employeeList = implode(', ', $employeeId);
    if($employeeList){
        $searchCondition = 'and hle.employee_id in (' . $employeeList . ')';
    }else{
        $searchCondition = '';
    }
     $sql = "select hle.employee_id, he.full_name, lms.leave_ename,hle.leave_id, hle.requested_date, 
        ...
        where 1 = 1 and hle.fiscal_year_id = {$fiscalId} " . $searchCondition ;
