...
class FlatValue extends HrisController {
    ...
    public function postFlatValueDetailAction() {
        try {
            ...
            //Grade ceiling validation starts
            $flat_id = $data['flatId'];
            $employee_id = $data['employeeId'];
            //$fiscal_year_id = $data['fiscalYearId'];

            if($flat_id == 16 || $flat_id == 23) {
                $res = [];
                $res = $detailRepo->getEMPPositionDetails($employee_id);

                if(!empty($res)) {
                    $position_id = $res[0]['POSITION_ID'];
                    $func_level_edesc = $res[0]['FUNCLEVEL'];
                    $position_type_ntechnical = 'Non-Technical';
                    $position_type_technical = 'Technical';
                }

                //NON Technical Grade Ceiling 
                if($flat_id == 16) {
                    if(!empty($res)) {
                        $resceiling = [];
                        $grade_ceiling = 0;
                        $grade_ceiling_ntechnical = 0;

                        $resceiling = $detailRepo->checkEmployeeGradeCeiling($position_id, $func_level_edesc, trim($position_type_ntechnical));

                        if(!empty($resceiling)) {
                            $grade_ceiling = $resceiling[0]['GRADE_CEILING_NO'];
                            if($grade_ceiling != 0) {
                                $grade_ceiling_ntechnical = $grade_ceiling;
                            }
                        }

                        if($grade_ceiling_ntechnical != 0) {
                            if($data['flatValue'] > $grade_ceiling_ntechnical) {
                                $data['flatValue'] = $grade_ceiling_ntechnical;
                            } else {
                                $data['flatValue'] = $data['flatValue'];
                            }
                        }
                    }
                ...
                //Technical Technical Grade Ceiling 
                if($flat_id == 23) {
                    if(!empty($res)) {
                        $resceiling_technical = [];
                        $grade_ceiling = 0;
                        $grade_ceiling_technical = 0;

                        $resceiling_technical = $detailRepo->checkEmployeeGradeCeiling($position_id, $func_level_edesc, trim($position_type_technical));

                        if(!empty($resceiling_technical)) {
                            $grade_ceiling_technical = $resceiling_technical[0]['GRADE_CEILING_NO'];
                            if($grade_ceiling_technical != 0) {
                                $grade_ceiling_technical = $grade_ceiling_technical;
                            }
                        }

                        if($grade_ceiling_technical != 0) {
                            if($data['flatValue'] > $grade_ceiling_technical) {
                                $data['flatValue'] = $grade_ceiling_technical;
                            } else {
                                $data['flatValue'] = $data['flatValue'];
                            }
                        }
    ...
    public function getBulkFlatValueDetailAction() {
        try {
            ...
            $employeeFilter = $postedData['employeeFilter'];
            $detailRepo = new FlatValueDetailRepo($this->adapter);
            $result = $detailRepo->getBulkFlatValuesDetailById($flatId, $fiscalYearId, $employeeFilter);
    ...
    public function getPositionWiseFlatValueAction() {
        try {
            ...
            $fiscalYearId = $postedData['fiscalYearId'];
            $detailRepo = new FlatValueDetailRepo($this->adapter);
            $result = $detailRepo->getPositionWiseFlatValue($flatId, $fiscalYearId, $positionId);
            $columns = $detailRepo->getColumns($flatId);
            return new JsonModel(['success' => true, 'data' => Helper::extractDbData($result), 'error' => '', 'columns' => Helper::extractDbData($columns)]);
        } catch (Exception $e) {
