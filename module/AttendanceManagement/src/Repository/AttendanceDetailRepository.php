...
class AttendanceDetailRepository implements RepositoryInterface {
    ...
    public function filterRecord($companyId, $branchId, $departmentId, $designationId, $positionId, $serviceTypeId, $serviceEventTypeId, $employeeTypeId, $genderId, $functionalTypeId, $employeeId, $fromDate = null, $toDate = null, $status = null, $presentStatus = null, $min = null, $max = null, $presentType = null, $locationId) {
        $boundedParams = [];
        ...
        if ($fromDate != null) {
            ...
            $fromDateCondition = "  AND A.ATTENDANCE_DT>=TO_DATE('$fromDate','DD-MON-YYYY') ";
            
        }
        if ($toDate != null) {
            $boundedParams['toDate'] = $toDate;
            $toDateCondition = "  AND A.ATTENDANCE_DT<=TO_DATE('$toDate','DD-MON-YYYY') ";
        }
        ...
        $sql = "
               ...
               Q.* FROM (SELECT 
                  ...
                  CASE
                    ...
                    THEN 
                    CASE when A.SP_ID is null
                    then 'Present'
                    else 'Present (' || (select sp_edesc from HRIS_SPECIAL_ATTENDANCE_SETUP where id = A.SP_ID) || ')'
                    end
                      --||LATE_STATUS_DESC(A.LATE_STATUS)
                  ...
                  TO_CHAR(S.END_TIME,'HH:MI AM')   AS END_TIME,
                   ...
                   END
                   AS SYSTEM_OVERTIME,
                  CASE WHEN OM.OVERTIME_HOUR is not null
                   THEN 
                  MIN_TO_HOUR(OM.OVERTIME_HOUR*60)
                   ELSE ''
...
//            new Expression("A.OUT_REMARKS AS OUT_REMARKS")], true);
        ...
        $select->where([
            ...
            "A.ATTENDANCE_DT=TO_DATE('" . $attendanceDt . "','DD-MON-YYYY')"
        ]);
        $statement = $sql->prepareStatementForSqlObject($select);
        $result = $statement->execute();
        return $result->current();
...
//        $select->columns([new Expression("TO_CHAR(A.ATTENDANCE_DT, 'DD-MON-YYYY') AS ATTENDANCE_DT"), new Expression("TO_CHAR(A.IN_TIME, 'HH:MI AM') AS IN_TIME"), new Expression("TO_CHAR(A.OUT_TIME, 'HH:MI AM') AS OUT_TIME"), new Expression("E.EMPLOYEE_ID AS EMPLOYEE_ID"), new Expression("A.ID AS ID"), new Expression("A.IN_REMARKS AS IN_REMARKS"), new Expression("A.OUT_REMARKS AS OUT_REMARKS")], true);
    ...
    public function fetchEmployeeShfitDetails($employeeId) {
        $sql = "SELECT
        TO_NVARCHAR( now(), 'HH:MI AM') AS" . ' "CURRENT_TIME",'.
        "TO_NVARCHAR(add_seconds(S.START_TIME,((S.LATE_IN*60)/86400)),
        'HH:MI AM') AS CHECKIN_TIME,
        TO_NVARCHAR(add_seconds(S.END_TIME,-((S.EARLY_OUT*60)/86400)),
        'HH:MI AM') AS CHECKOUT_TIME,
        TO_NVARCHAR(add_seconds(S.HALF_DAY_END_TIME,-((S.EARLY_OUT*60)/86400)),
        'HH:MI AM') AS HALF_DAY_CHECKOUT_TIME,
        ESA.*,
       S.* 
   FROM HRIS_EMPLOYEES E join HRIS_EMPLOYEE_SHIFT_ASSIGN ESA on (ESA.EMPLOYEE_ID=E.EMPLOYEE_ID) JOIN HRIS_SHIFTS S ON (S.SHIFT_ID=ESA.SHIFT_ID) 
   WHERE E.EMPLOYEE_ID=$employeeId 
   AND ESA.STATUS='E' 
   AND ESA.MODIFIED_DT IS NULL 
   AND (TO_DATE(CURRENT_DATE,
        'YYYY-MM-DD') BETWEEN TO_DATE(S.START_DATE,
        'YYYY-MM-DD') 
       AND TO_DATE(S.END_DATE,
        'YYYY-MM-DD'))
        ";
        $statement = $this->adapter->query($sql);
        $result = $statement->execute();
...
FROM (SELECT 
                 ROW_NUMBER() over (ORDER BY  E.SENIORITY_LEVEL  ASC,P.LEVEL_NO  ASC,E.JOIN_DATE  ASC,A.ATTENDANCE_DT DESC ,A.IN_TIME ASC)                                           AS R,
                  ...
                  BS_DATE(TO_CHAR(A.ATTENDANCE_DT, 'YYYY-MM-DD')) AS ATTENDANCE_DT_N,
                  INITCAP(TO_CHAR(A.IN_TIME, 'HH:MI AM'))          AS IN_TIME,
                  INITCAP(TO_CHAR(A.OUT_TIME, 'HH:MI AM'))         AS OUT_TIME,
                  A.IN_REMARKS                                     AS IN_REMARKS,
                  A.OUT_REMARKS                                    AS OUT_REMARKS,
    ...
    public function fetchDailyPerformanceReport($data){
      ...
      $sql .= $data['fromDate'] == null ? "current_date " : " '$fromDate' "  ; 
      $sql .= " and ";
      $sql .= $data['toDate'] == null ? "current_date " : " '$toDate' " ; 
    ...
    public function filterRecordRaw($employeeId, $fromDate = null, $toDate = null){
        ...
        $searchConditon .= $toDate != null ? "AND A.ATTENDANCE_DT<=TO_DATE('" . $toDate . "','DD-MM-YYYY')" : "AND A.ATTENDANCE_DT <= current_date " ;
        $sql = "SELECT
    e.employee_code,
    e.full_name,
    a.attendance_dt,
    ...
    INITCAP(TO_VARCHAR(A.attendance_time, 'HH:MI AM'))   attendance_time,
...
WHERE
    ...
    public function fetchEmpShiftDtlByAttendance($employeeId){
        $sql = "SELECT INITCAP(TO_CHAR(SYSDATE, 'HH:MI AM')) AS CURRENT_TIME,
            INITCAP(TO_VARCHAR(S.START_TIME+((S.LATE_IN*60)/86400), 'HH:MI AM')) AS CHECKIN_TIME, 
            INITCAP(TO_VARCHAR(S.END_TIME-((S.EARLY_OUT*60)/86400), 'HH:MI AM')) AS CHECKOUT_TIME,
            INITCAP(TO_VARCHAR(S.HALF_DAY_END_TIME-((S.EARLY_OUT*60)/86400), 'HH:MI AM')) AS HALF_DAY_CHECKOUT_TIME,
    ...
    public function filterAttendanceRecord($companyId, $branchId, $departmentId, $designationId, $positionId, $serviceTypeId, $serviceEventTypeId, $employeeTypeId, $genderId, $functionalTypeId, $employeeId, $fromDate = null, $toDate = null, $status = null, $presentStatus = null, $min = null, $max = null, $presentType = null, $locationId, $monthId) {
        ...
        $sql = "
        select E.id_pan_no, E.employee_code, E.full_name, ifnull(EP.total_present,0) as total_days, ifnull(ET.total_present,0) as Total_travel ,
        (ifnull(EP.total_present,0) + ifnull(EWH.total_present,0) + ifnull(EWD.total_present,0)) as Lunch_allowance, ifnull(EH.total_present,0) as Total_Holiday , ifnull(EWH.total_present,0) as total_working_on_holiday,
        ifnull(EWD.total_present,0) as total_working_on_dayoff, 
        ifnull(TWD.total_present,0) as total_working_days,
        ifnull(AB.total_present,0) as absent_days,
        ifnull(SP.total_present,0) as special_present,
        0 as bato_myaad,
        0 as unpaid_leave
        ,ifnull(LV.total_present,0) as casual_leave, ifnull(HL.total_present,0) as House_leave , 
        ifnull(CRL.total_present,0) as Creamation_leave
        , ifnull(RL.total_present,0) as replacement_leave, 
        ifnull(SL.total_present,0) as Sick_leave, 
        ifnull(ML.total_present,0) as Maternal_leave
        from hris_employees E 
        left join (select employee_id, count(employee_id) as total_present from hris_attendance_detail
        where overall_status = 'PR' 
        and attendance_dt between 
        (select from_date from hris_month_code where month_id = {$monthId}) and (select to_date from hris_month_code where month_id = {$monthId})     
        group by employee_id) EP on (EP.employee_id = E.employee_id)
        
        left join (select employee_id, count(employee_id) as total_present from hris_attendance_detail
        where overall_status = 'TV' 
        and attendance_dt between 
        (select from_date from hris_month_code where month_id = {$monthId}) and (select to_date from hris_month_code where month_id = {$monthId})     
        group by employee_id) ET on (ET.employee_id = E.employee_id)

        left join (select employee_id, count(employee_id) as total_present from hris_attendance_detail
        where overall_status = 'AB' 
        and attendance_dt between 
        (select from_date from hris_month_code where month_id = {$monthId}) and (select to_date from hris_month_code where month_id = {$monthId})     
        group by employee_id) AB on (AB.employee_id = E.employee_id)
        
        left join (select employee_id, count(employee_id) as total_present from hris_attendance_detail
        where overall_status in ('HD','DO') 
        and attendance_dt between 
        (select from_date from hris_month_code where month_id = {$monthId}) and (select to_date from hris_month_code where month_id = {$monthId})     
        group by employee_id) EH on (EH.employee_id = E.employee_id)

        left join (select employee_id, count(employee_id) as total_present from hris_attendance_detail
        where overall_status in ('SP') 
        and attendance_dt between 
        (select from_date from hris_month_code where month_id = {$monthId}) and (select to_date from hris_month_code where month_id = {$monthId})     
        group by employee_id) SP on (SP.employee_id = E.employee_id)
        
        left join (select employee_id, count(employee_id) as total_present from hris_attendance_detail
        where overall_status = 'WH' 
        and attendance_dt between 
        (select from_date from hris_month_code where month_id = {$monthId}) and (select to_date from hris_month_code where month_id = {$monthId})     
        group by employee_id) EWH on (EWH.employee_id = E.employee_id)

        left join (select employee_id, count(employee_id) as total_present from hris_attendance_detail
        where overall_status = 'WD' 
        and attendance_dt between 
        (select from_date from hris_month_code where month_id = {$monthId}) and (select to_date from hris_month_code where month_id = {$monthId})     
        group by employee_id) EWD on (EWD.employee_id = E.employee_id)
        
        left join (select employee_id, count(employee_id) as total_present from hris_attendance_detail
        where  attendance_dt between 
        (select from_date from hris_month_code where month_id = {$monthId}) and (select to_date from hris_month_code where month_id = {$monthId})     
        group by employee_id) TWD on (TWD.employee_id = E.employee_id)
        
        left join (select employee_id, count(employee_id) as total_present from hris_attendance_detail
        where overall_status  in  ('LV') and leave_id  in ( select leave_id from hris_leave_master_setup where leave_code ='CASULEV' and status='E')
        and attendance_dt between 
        (select from_date from hris_month_code where month_id = {$monthId}) and (select to_date from hris_month_code where month_id = {$monthId})     
        group by employee_id) LV on (LV.employee_id = E.employee_id)
        --
        left join (select employee_id, count(employee_id) as total_present from hris_attendance_detail
        where overall_status  in  ('LV') and leave_id  in ( select leave_id from hris_leave_master_setup where leave_code ='HOUSLEV' and status='E') --HOUSLEV
        and attendance_dt between 
        (select from_date from hris_month_code where month_id = {$monthId}) and (select to_date from hris_month_code where month_id = {$monthId})     
        group by employee_id) HL on (HL.employee_id = E.employee_id)
        
        left join (select employee_id, count(employee_id) as total_present from hris_attendance_detail
        where overall_status  in  ('LV') and leave_id  in ( select leave_id from hris_leave_master_setup where leave_code ='CREMLEV' and status='E')--CREMLEV
        and attendance_dt between 
        (select from_date from hris_month_code where month_id = {$monthId}) and (select to_date from hris_month_code where month_id = {$monthId})     
        group by employee_id) CRL on (CRL.employee_id = E.employee_id)
        
        left join (select employee_id, count(employee_id) as total_present from hris_attendance_detail
        where overall_status  in  ('LV') and leave_id  in ( select leave_id from hris_leave_master_setup where leave_code ='REPLLEV' and status='E') --REPLLEV
        and attendance_dt between 
        (select from_date from hris_month_code where month_id = {$monthId}) and (select to_date from hris_month_code where month_id = {$monthId})     
        group by employee_id) RL on (RL.employee_id = E.employee_id)
        
        left join (select employee_id, count(employee_id) as total_present from hris_attendance_detail
        where overall_status  in  ('LV') and leave_id  in ( select leave_id from hris_leave_master_setup where leave_code ='SICKLEV' and status='E') --SICKLEV
        and attendance_dt between 
        (select from_date from hris_month_code where month_id = {$monthId}) and (select to_date from hris_month_code where month_id = {$monthId})     
        group by employee_id) SL on (SL.employee_id = E.employee_id)
        
        left join (select employee_id, count(employee_id) as total_present from hris_attendance_detail
        where overall_status  in  ('LV') and leave_id  in ( select leave_id from hris_leave_master_setup where leave_code ='MTRNLEV' and status='E') --MTRNLEV
        and attendance_dt between 
        (select from_date from hris_month_code where month_id = {$monthId}) and (select to_date from hris_month_code where month_id = {$monthId})     
        group by employee_id) ML on (ML.employee_id = E.employee_id)        
