...
class AttendanceStatusRepository extends HrisRepository {
    ...
    public function getFilteredRecord($data, $recomApproveId) {
        ...
        $sql = "SELECT DISTINCT AR.ID                                             AS ID,
                  ...
                  BS_DATE((AR.ATTENDANCE_DT))               AS ATTENDANCE_DT_BS,
                  ...
                  BS_DATE((AR.REQUESTED_DT))                AS REQUESTED_DT_BS,
...
//                echo $sql;
    ...
    public function getAttenReqList($data): array {
        ...
        $boundedParams = [];
        $searchCondition = EntityHelper::getSearchConditonBounded($companyId, $branchId, $departmentId, $positionId, $designationId, $serviceTypeId, $serviceEventTypeId, $employeeTypeId, $employeeId, null, $location, $functionalTypeId);
        $boundedParams = array_merge($boundedParams, $searchCondition['parameter']);
        // print_r($boundedParams);die;
        $statusCondition = "";
        $fromDateCondition = "";
        $toDateCondition = "";
        ...
        if ($attendanceRequestStatusId != -1) {
            ...
            $statusCondition = "AND AR.STATUS = ?";
            
        }

        if ($fromDate != null) {
            $boundedParams['fromDate'] = $fromDate;
            $fromDateCondition = " AND AR.ATTENDANCE_DT>= TO_DATE(?,'DD-MON-YYYY') ";
            
        }

        if ($toDate != null) {
            $boundedParams['toDate'] = $toDate;
            $toDateCondition = "AND AR.ATTENDANCE_DT<= TO_DATE(?,'DD-MON-YYYY') ";
