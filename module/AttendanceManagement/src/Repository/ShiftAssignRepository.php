...
EOT;
    ...
    public function bulkAdd($employeeId, $shiftId, $fromDate, $toDate, $createdBy) {
        ...
        $sql = <<<EOT
                CALL
                  HRIS_SHIFT_ADD(?,?,to_date(?,'DD-Mon-YYYY'),to_date(?,'DD-Mon-YYYY'),?);
EOT;
    ...
    public function getNepaliDate($engDate){
      $sql = "select bs_date(to_date('{$engDate}', 'DD-Mon-YYYY')) as NEPDATE from dummy";
      $statement = $this->adapter->query($sql);
      $result = $statement->execute();
      return Helper::extractDbData($result)[0]['NEPDATE'];
    }
