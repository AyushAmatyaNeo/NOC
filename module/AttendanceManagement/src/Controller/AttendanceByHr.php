    ...
    */
        ...
        try {
            if ($request->isPost()) {
                ...
                if ($this->form->isValid()) {
                    ...
                    //$data['attendanceDt'] = Helper::getExpressionDate( $data['attendanceDt']);
    ...
    public function pullAttendanceAction() {
        try {
            ...
            $results = $this->repository->filterRecord($companyId, $branchId, $departmentId, $designationId, $positionId, $serviceTypeId, $serviceEventTypeId, $employeeTypeId, $genderId, $functionalTypeId, $employeeId, $fromDate, $toDate, $status, $presentStatus, null, null, $presentType, $locationId);
            $result = [];
            $result['success'] = true;
...
//    }
    ...
    public function registerAttendanceAction() {
        $request = $this->getRequest();
        if ($request->isPost()) {
            ...
            $attendanceModel->attendanceDt = Helper::getcurrentExpressionDate();
            $attendanceModel->attendanceTime = new Expression("CURRENT_TIMESTAMP");
            ...
            // echo('<pre>');print_r($attendanceModel);die;
            $attendanceRepo->add($attendanceModel);
            $this->flashmessenger()->addMessage("Attendance Registered Successfully!!");
            return $this->redirect()->toRoute("attendancebyhr", ['action' => 'registerAttendance']);
