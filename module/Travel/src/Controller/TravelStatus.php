    ...
    public function indexAction() {
        ...
        if ($request->isPost()) {
            try {
                ...
                return new JsonModel(['success' => true, 'data' => $list, 'error' => '', 'errorMessage' => '']);
            } catch (Exception $e) {
...
//                    'itnaryCodeList'=>[], 
    ...
    public function deleteApprovedTravelAction(){
        $request = $this->getRequest();
        if ($request->isPost()) {
            $travelRequest = new TravelRequest();
            $getData = $request->getPost();
            $id = $getData['travelId'];
            try{
                $errorMessage = $this->travelStatusRepository->deleteApprovedTravel($id);
                if($errorMessage == "Can't apply"){
                    return new JsonModel(['success' => false, 'data' => [], 'error' => 'Please cancel expense request first!!!']);
                }else{
