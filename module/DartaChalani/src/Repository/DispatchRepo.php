...
class DispatchRepo extends HrisRepository {
    ...
    public function fetchSearchData($data){
        ...
        if($data['fiscalYearId'] != null && $data['fiscalYearId'] != -1 && $data['fiscalYearId'] != " ") {
            $fiscalYearCondition = " and d.DISPATCH_DATE between (select start_date from hris_fiscal_years where fiscal_year_id = {$data['fiscalYearId']}) 
            and (select end_date from hris_fiscal_years where fiscal_year_id = {$data['fiscalYearId']})";
        }
        ...
        {$toOfficeCodeCondition} {$toLocationCodeCondition} {$fiscalYearCondition}
        order by d.dispatch_id desc";
