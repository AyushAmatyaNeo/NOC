...
class TravelExpensesRepository extends HrisRepository implements RepositoryInterface {
    ...
    function fetchDomesticById($id){
        ...
        order by hte.DEPARTURE_DT";
        $boundedParams['id'] = $id;
        // print_r($sql);print_r($boundedParams);die;
        return $this->rawQuery($sql, $boundedParams);
    }

    function fetchAllInOrder($id){
        $boundedParams = [];
        ...
        hte.HALF_DAY_FLAG,
        ctc.domestic_Type,
        ctc.international_Type,
         round(hte.amount/ctc.rate,2) as unit, days_between(hte.departure_dt, hte.arraival_dt) +1 as noOfDays from hris_travel_expense hte 
        left join hris_class_travel_config ctc on (ctc.config_id = hte.config_id)
        where hte.travel_id = {$id} and hte.status='E' and hte.config_id in (select config_id from hris_class_travel_config)
        order by hte.DEPARTURE_DT";
        $boundedParams['id'] = $id;
        // print_r($sql);print_r($boundedParams);die;
        return $this->rawQuery($sql, $boundedParams);
