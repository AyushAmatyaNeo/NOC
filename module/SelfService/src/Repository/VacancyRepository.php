...
select functional_level_id from hris_employees where employee_id = $empId)))"]);
    ...
    public function admitCardVacancy($uid, $appid){
        ...
        $sql  = "SELECT 
                    ...
                    RV.AD_NO, RV.AD_NO_NEPALI,
                    HUR.FIRST_NAME,HUR.MIDDLE_NAME,HUR.LAST_NAME,
                    HUR.ID_CITIZENSHIP_NO, HUR.ID_CITIZENSHIP_ISSUE_DATE,
                    VU.USER_ID,
                    HUR.EMPLOYEE_ID,

                    HD.DESIGNATION_TITLE, HD.DESIGNATION_NTITLE,
                    HFL.FUNCTIONAL_LEVEL_EDESC,
                    HST.SERVICE_TYPE_NAME, HST.SERVICE_TYPE_NAME_NEPALI,
                    HET.SERVICE_EVENT_NAME, HET.SERVICE_EVENT_NAME_NEPALI,
                    ...
                    VSN.NAME AS NEPALI_NAME


                FROM HRIS_REC_VACANCY_APPLICATION AS NV

                LEFT JOIN HRIS_REC_VENUE_ASSIGN FPVA ON (FPVA.VENUE_ASSIGN_ID = NV.FIRST_PAPER_VENUE_ASSIGN_ID)
                LEFT JOIN HRIS_REC_VENUE_SETUP FPV ON FPV.VENUE_SETUP_ID = FPVA.VENUE_SETUP_ID


                LEFT JOIN HRIS_REC_VENUE_ASSIGN SPVA ON (SPVA.VENUE_ASSIGN_ID = NV.SECOND_PAPER_VENUE_ASSIGN_ID)
                LEFT JOIN HRIS_REC_VENUE_SETUP SPV ON SPV.VENUE_SETUP_ID = SPVA.VENUE_SETUP_ID
                ...
                LEFT JOIN HRIS_REC_VACANCY_USER_NEPALI AS VSN ON (VSN.USER_ID = VU.USER_ID AND VSN.APPLICATION_TYPE='INTERNAL') 
