...
END;";
    ...
    public function getAllByEmployeeId($employeeId) {
        ...
        $select->columns([
            new Expression("WD.ID AS ID"),
            new Expression("WD.EMPLOYEE_ID AS EMPLOYEE_ID"),
            new Expression("INITCAP(TO_CHAR(WD.REQUESTED_DATE, 'YYYY-MM-DD')) AS REQUESTED_DATE_AD"),
            new Expression("BS_DATE(TO_CHAR(WD.REQUESTED_DATE, 'YYYY-MM-DD')) AS REQUESTED_DATE_BS"),
            new Expression("INITCAP(TO_CHAR(WD.FROM_DATE, 'YYYY-MM-DD')) AS FROM_DATE_AD"),
            new Expression("BS_DATE(TO_CHAR(WD.FROM_DATE, 'YYYY-MM-DD')) AS FROM_DATE_BS"),
            new Expression("INITCAP(TO_CHAR(WD.TO_DATE, 'YYYY-MM-DD')) AS TO_DATE_AD"),
            new Expression("BS_DATE(TO_CHAR(WD.TO_DATE, 'YYYY-MM-DD')) AS TO_DATE_BS"),
            new Expression("WD.DURATION AS DURATION"),
            new Expression("WD.REMARKS AS REMARKS"),
            new Expression("WD.STATUS AS STATUS"),
            new Expression("LEAVE_STATUS_DESC(WD.STATUS) AS STATUS_DETAIL"),
            new Expression("WD.RECOMMENDED_BY AS RECOMMENDED_BY"),
            new Expression("INITCAP(TO_CHAR(WD.RECOMMENDED_DATE, 'YYYY-MM-DD')) AS RECOMMENDED_DATE"),
            new Expression("WD.RECOMMENDED_REMARKS AS RECOMMENDED_REMARKS"),
            new Expression("WD.APPROVED_BY AS APPROVED_BY"),
            new Expression("INITCAP(TO_CHAR(WD.APPROVED_DATE, 'YYYY-MM-DD')) AS APPROVED_DATE"),
            new Expression("WD.APPROVED_REMARKS AS APPROVED_REMARKS"),
            new Expression("INITCAP(TO_CHAR(WD.MODIFIED_DATE, 'YYYY-MM-DD')) AS MODIFIED_DATE"),
        ...
        $select->where([
            "(days_between( WD.REQUESTED_DATE,CURRENT_DATE)) < (
                      CASE
                        WHEN WD.STATUS = 'C'
                        THEN 20
                        ELSE 365
