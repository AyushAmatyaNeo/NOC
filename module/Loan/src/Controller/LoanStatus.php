...
class LoanStatus extends HrisController {
    ...
    public function indexAction() {
        ...
        $loanFormElement->setAttributes(["id" => "loanId", "class" => "form-control reset-field", "multiple" => "multiple"]);
    ...
    public function viewAction() {
        ...
        if (!$request->isPost()) {
            ...
            $loanRequest->loanRequestId = $id;
            $loanRequest->exchangeArrayFromDB($detail);
            // $this->financialImpact($loanRequest);
    ...
    public function payDetailsAction(){
        $id = (int) $this->params()->fromRoute('id');
        ...
        if ($request->isPost()) {
            $id = (int) $this->params()->fromRoute('id');
            $data = $this->loanStatusRepository->payDetailsList($id);
    ...
    public function skipAction(){
        ...
        if($paidFlag == 'N' && $amount != 0){
            ...
            $this->flashmessenger()->addMessage('Skip is not possible. Loan has already been paid or skipped this month.');
        }
