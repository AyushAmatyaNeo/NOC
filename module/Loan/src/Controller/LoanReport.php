...
class LoanReport extends HrisController {
    ...
    public function loanPaymentReportAction(){
        $request = $this->getRequest();
        if ($request->isPost()){
            try {
                $data = $request->getPost();
                $result = Helper::extractDbData($this->repository->getLoanPaymentReport($data));
                return new JsonModel(['success' => true, 'data' => $result, 'message' => null]);
            } catch (Exception $e) {
                return new JsonModel(['success' => false, 'data' => null, 'message' => $e->getMessage()]);
            }
        } 
        $loanList = Helper::extractDbData($this->repository->getLoanlist());
        $searchValues = ApplicationHelper::getSearchData($this->adapter);
        $$searchValues['employee'] = Helper::extractDbData($this->repository->getAllEmployees());
