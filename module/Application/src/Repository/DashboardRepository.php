...
class DashboardRepository {
    ...
    public function fetchEmployeeDashboardDetail($employeeId, $startDate, $endDate) {
        $sql = "
            ...
            FROM
              (SELECT EMP.EMPLOYEE_ID, EMP.FULL_NAME, EMP.GENDER_ID, EMP.COMPANY_ID, EMP.BRANCH_ID, EMP.EMAIL_OFFICIAL, 
              EMP.EMAIL_PERSONAL, TO_CHAR(EMP.JOIN_DATE, 'DD-MON-YYYY') JOIN_DATE, 
              FLOOR(DAYS_BETWEEN(EMP.JOIN_DATE, current_date) / 365) AS SERVICE_YEARS, 
              MOD(MONTHS_BETWEEN(EMP.JOIN_DATE, current_date), 12) AS SERVICE_MONTHS, 
              (DAYS_BETWEEN(EMP.JOIN_DATE, current_date) - 
              FLOOR(DAYS_BETWEEN(EMP.JOIN_DATE, current_date) / 365)*365) -
              MOD(MONTHS_BETWEEN(EMP.JOIN_DATE, current_date), 12) * 30.42
               AS SERVICE_DAYS,
              DSG.DESIGNATION_TITLE, EFL.FILE_PATH FROM HRIS_EMPLOYEES EMP 
              left join HRIS_DESIGNATIONS DSG on (EMP.DESIGNATION_ID = DSG.DESIGNATION_ID) 
              left join HRIS_EMPLOYEE_FILE EFL on (EMP.PROFILE_PICTURE_ID = EFL.FILE_CODE) 
              where EMP.RETIRED_FLAG = 'N' AND EMP.EMPLOYEE_ID = {$employeeId}
              ) EMPLOYEE_TBL
            LEFT JOIN
              (SELECT 
              count(case when atten_tbl.OVERALL_STATUS in ('PR', 'WD', 'WH', 'TP', 'LP', 'VP') then atten_tbl.OVERALL_STATUS end) PRESENT_DAY,
              count(case when atten_tbl.OVERALL_STATUS in ('AB', 'BA', 'LA') then atten_tbl.OVERALL_STATUS end) ABSENT_DAY,
              count(case when atten_tbl.OVERALL_STATUS in ('LV') then atten_tbl.OVERALL_STATUS end) LEAVE,
              count(case when atten_tbl.OVERALL_STATUS in ('WH') then atten_tbl.OVERALL_STATUS end) WOH,
              count(case when atten_tbl.OVERALL_STATUS in ('TV') then atten_tbl.OVERALL_STATUS end) TOUR,
              count(case when atten_tbl.OVERALL_STATUS in ('TN', 'TP') then atten_tbl.OVERALL_STATUS end) TRAINING
...
FROM HRIS_ATTENDANCE_DETAIL ad
                ...
                WHERE (ad.employee_id = {$employeeId} and ad.ATTENDANCE_DT BETWEEN TO_DATE('{$startDate}', 'YYYY-MM-DD') AND TO_DATE('{$endDate}', 'DD-MON-YYYY'))
                ) ATTEN_TBL--PIVOT (COUNT(OVERALL_STATUS) FOR OVERALL_STATUS IN ('DO','HD','LV','TV','TN','PR','AB','WD','WH','BA','LA','TP','LP','VP','CU'))
              ) ATTEN_TBL
            ON (1=1)
            LEFT JOIN
              (SELECT 
              count(case when LATE_ATTEN_TBL.LATE_STATUS in ('L', 'B', 'Y') then LATE_ATTEN_TBL.LATE_STATUS end) LATE_IN,
              count(case when LATE_ATTEN_TBL.LATE_STATUS in ('E', 'B') then LATE_ATTEN_TBL.LATE_STATUS end) EARLY_OUT,
              count(case when LATE_ATTEN_TBL.LATE_STATUS in ('X', 'Y') then LATE_ATTEN_TBL.LATE_STATUS end) MISSED_PUNCH
              from (SELECT EMPLOYEE_ID,
                  LATE_STATUS
                FROM HRIS_ATTENDANCE_DETAIL
                WHERE (employee_id = {$employeeId} and ATTENDANCE_DT BETWEEN TO_DATE('{$startDate}', 'YYYY-MM-DD') AND TO_DATE('{$endDate}', 'DD-MON-YYYY'))
                ) LATE_ATTEN_TBL --PIVOT (COUNT(LATE_STATUS) FOR LATE_STATUS IN ('L','E','B','N','X','Y'))
              ) LATE_ATTEN_TBL
            ON (1=1)
          ";
    ...
    public function fetchUpcomingHolidays($employeeId = null) {
        if ($employeeId == null) {
            $sql = "SELECT HM.HOLIDAY_ID,
                  HM.HOLIDAY_ENAME,
                  TO_CHAR(HM.START_DATE,'Day, DD Month') START_DATE,
                  TO_CHAR(HM.END_DATE,'Day, DD Month') END_DATE,
                  HM.HALFDAY,
                  TO_CHAR(HM.START_DATE, 'DAY') WEEK_DAY,
               DAYS_BETWEEN(current_date, HM.START_DATE)  DAYS_REMAINING
                FROM HRIS_HOLIDAY_MASTER_SETUP HM
                WHERE  HM.START_DATE >  current_date ORDER BY HM.START_DATE";
        } else {
            $sql = "SELECT HM.HOLIDAY_ID,
                  HM.HOLIDAY_ENAME,
                  TO_CHAR(HM.START_DATE,'Day, DD Month') START_DATE,
                ...
                WHERE EH.EMPLOYEE_ID={$employeeId} AND HM.START_DATE > CURRENT_DATE ORDER BY HM.START_DATE";
        }
    ...
    public function fetchEmployeeNotice($employeeId = null) {
        ...
        $sql = "SELECT N.NEWS_ID,
                      ...
                      N.NEWS_EDESC
                    FROM HRIS_NEWS N   
                    WHERE ( CURRENT_DATE BETWEEN NEWS_DATE AND NEWS_EXPIRY_DT ) 
                    AND N.STATUS = 'E'
                   {$where}
                    ";
        $statement = $this->adapter->query($sql);
    ...
    public function fetchEmployeeTask($employeeId) {
        $sql = "SELECT TSK.TASK_ID,
                    ...
                    WHERE 1 = 1
                        ...
                        AND (TSK.END_DATE> CURRENT_DATE OR TSK.STATUS = 'O')";
...
FROM
    (
        SELECT
            ...
            emp.full_name,
            emp.birth_date,
            TO_CHAR(emp.birth_date,'DD-MON') emp_birth_date,
        ...
        FROM
            hris_employees emp
        WHERE
                TO_CHAR(emp.birth_date,'MMDD') = TO_CHAR(CURRENT_DATE,'MMDD')
            AND
                emp.retired_flag = 'N'
            AND
                emp.status = 'E'
     ...
     hris_designations dsg on ( emp.designation_id = dsg.designation_id) 
     left join 
    hris_employee_file efl on ( emp.profile_picture_id = efl.file_code) 
    left join 
    hris_branches b on (emp.branch_id = b.branch_id ) left join
    
    hris_positions p on (emp.position_id = p.position_id )
WHERE
       1=1
    ...
    public function fetchEmployeeCalendarData($employeeId, $startDate, $endDate) {
        $sql = "SELECT TO_CHAR(ATN.ATTENDANCE_DT, 'YYYY-MM-DD') MONTH_DAY,
                  ...
                  CASE
                    ...
                    || ''
                    ELSE 'Present ' 
                    || LATE_STATUS_DESC(ATN.LATE_STATUS) END) 
...
";
    ...
    public function fetchGenderHeadCount() {
        $sql = "SELECT COUNT (*) HEAD_COUNT, HE.GENDER_ID, HG.GENDER_NAME
                    FROM HRIS_EMPLOYEES HE left join HRIS_GENDERS HG
                    on (HE.GENDER_ID = HG.GENDER_ID)
                   WHERE 
                      HG.STATUS = 'E'
    ...
    public function fetchDepartmentHeadCount() {
        $sql = "SELECT COUNT (*) HEAD_COUNT, HD.DEPARTMENT_ID , HD.DEPARTMENT_NAME
                    FROM HRIS_EMPLOYEES HE left join 
                    HRIS_DEPARTMENTS HD on (HE.DEPARTMENT_ID = HD.DEPARTMENT_ID)
                   AND HD.STATUS = 'E'
                   AND HE.RETIRED_FLAG = 'N'
                   --AND HE.COMPANY_ID = :V_COMPANY_ID
    ...
    public function fetchLocationHeadCount() {
        $sql = "SELECT COUNT (*) HEAD_COUNT, HB.BRANCH_ID , HB.BRANCH_NAME
                    FROM HRIS_EMPLOYEES HE left join HRIS_BRANCHES HB
                    on (HE.BRANCH_ID = HB.BRANCH_ID) where
                    HB.STATUS = 'E'
                   AND HE.RETIRED_FLAG = 'N'
                   --AND HE.COMPANY_ID = :V_COMPANY_ID
    ...
    public function fetchDepartmentAttendance() {
        $sql = "SELECT * FROM (
                    ...
                    WHERE HD.DEPARTMENT_ID = HE.DEPARTMENT_ID
                      ...
                      AND HAD.ATTENDANCE_DT = current_date
        ...
        $sql = "
                ...
                JOIN
                  ...
                  FROM HRIS_JOB_HISTORY JH,
                    ...
                    AND days_between(current_date, end_date)<=15
...
";
    ...
    public function allNewsTypeWise($typeId, $employeeId) {
        $sql = "SELECT N.NEWS_ID,
                ...
                AND (CURRENT_DATE BETWEEN N.NEWS_DATE AND N.NEWS_EXPIRY_DT)
...
LEFT JOIN HRIS_EMPLOYEE_LEAVE_REQUEST LR ON (LR.EMPLOYEE_ID=AD.EMPLOYEE_ID AND CURRENT_DATE BETWEEN LR.START_DATE AND LR.END_DATE )
LEFT JOIN HRIS_BRANCHES B ON (B.BRANCH_ID=E.BRANCH_ID)
WHERE AD.ATTENDANCE_DT=CURRENT_DATE
