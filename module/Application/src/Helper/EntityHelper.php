...
class EntityHelper
{
    ...
    public static function getSearchData($adapter, $getDisabled = false)
    {
        ...
        $employeeList = self::getTableList($adapter, HrEmployees::TABLE_NAME, [
            ...
            new Expression(HrEmployees::LOCATION_ID . " AS " . HrEmployees::LOCATION_ID),
        ...
        $searchValues = [
            ...
            'gender' => $genderList,
            'employeeType' => [['EMPLOYEE_TYPE_KEY' => 'R', 'EMPLOYEE_TYPE_VALUE' => 'Regular'], ['EMPLOYEE_TYPE_KEY' => 'C', 'EMPLOYEE_TYPE_VALUE' => 'Contract'], ['EMPLOYEE_TYPE_KEY' => 'O', 'EMPLOYEE_TYPE_VALUE' => 'Out Source'], ['EMPLOYEE_TYPE_KEY' => 'D', 'EMPLOYEE_TYPE_VALUE' => 'Daily Wages']],
    ...
    public static function conditionBuilderBounded($colValue, $colName, $conditonType, $isString = false, $parentQuery = false)
    {
        ...
        if (gettype($colValue) === "array") {
            ...
            if ($parentQuery) {
                $value = str_replace('INVALUES', $value, $parentQuery);
                return " {$conditonType} {$colName} IN (?)";
            } else {
                $sql = "{$conditonType} {$colName} = ? ";
            }
            $returnData['sql'] = $sql;
            $returnData['parameter'] = $parameterData;
            return $returnData;
    ...
    public static function getSearchConditon($companyId, $branchId, $departmentId, $positionId, $designationId, $serviceTypeId, $serviceEventTypeId, $employeeTypeId, $employeeId, $genderId = null, $locationId = null, $functionalTypeId = null)
    {
        ...
        if ($departmentId != null && $departmentId != -1) {
            ...
            department_id
         FROM HIERARCHY ( 
         SOURCE ( SELECT department_id, department_id as node_id, 
         parent_department as parent_id 
                     FROM hris_departments WHERE status = 'E' 
                     ORDER BY node_id ) start where department_id in (INVALUES)
                          ORPHAN IGNORE
         CACHE FORCE ))";
    ...
    public static function getQualificationStatusSelectElement()
    {
        $statusFormElement = new Select2();
        $statusFormElement->setName("status");
        $status = array(
        ...
        $statusFormElement->setValueOptions($status);
        $statusFormElement->setAttributes(["id" => "QualificationId", "class" => "form-control", "multiple" => "multiple"]);
        $statusFormElement->setLabel("Qualification");
        return $statusFormElement;
    }
  ...
  FROM (SELECT EXCEPTIONAL  FROM  HRIS_DEPARTMENTS WHERE DEPARTMENT_ID IN  ({$depVal}))
   CONNECT BY  REGEXP_SUBSTR(EXCEPTIONAL, '[^,]+', 1, LEVEL) IS NOT NULL)");
    ...
    public static function getSearchConditonHana($companyId, $branchId, $departmentId, $positionId, $designationId, $serviceTypeId, $serviceEventTypeId, $employeeTypeId, $employeeId, $genderId = null, $locationId = null, $functionalTypeId = null)
    {
        $conditon = "";
        if ($companyId != null && $companyId != -1) {
            $conditon .= self::conditionBuilder($companyId, "E.COMPANY_ID", "AND");
