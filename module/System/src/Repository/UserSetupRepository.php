...
class UserSetupRepository implements RepositoryInterface {
    ...
    public function getEmployeeList($employeeId = null) {
        ...
        if ($employeeId != null) {
            ...
            $sql = " 
        SELECT EMPLOYEE_CODE||'-'||FULL_NAME  AS FULL_NAME,EMPLOYEE_ID FROM HRIS_EMPLOYEES WHERE STATUS='E' AND EMPLOYEE_ID IN (?)";
    ...
    public function fetchById($id) {
        ...
        $select->columns([
            ...
            new Expression("PASSWORD AS PASSWORD"),
    ...
    public function updateByEmpId($employeeId, $password) {
        $this->tableGateway->update([UserSetup::PASSWORD => $password,UserSetup::FIRST_TIME=>'N', UserSetup::MODIFIED_DT=> new Expression2('CURRENT_DATE')], [UserSetup::EMPLOYEE_ID => $employeeId]);
    }
    ...
    public function checkUserNameAvailability($userName,$userId) {
        ...
        $sql = "SELECT * FROM HRIS_USERS WHERE LOWER(USER_NAME)=LOWER(?) ";
