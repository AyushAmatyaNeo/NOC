...
class EmployeeController extends HrisController {
    ...
    public function initializeMultipleForm() {
        ...
        if (!$this->formOne) {
            ...
            $ethnicityId = $this->formOne->get('ethnicityId');
            ...
            $ethnicityList = ApplicationHelper::getTableKVList($this->adapter, 'HRIS_ETHNICITIES', 'ETHNICITY_ID', ['ETHNICITY'], null, null, true);

            $genderId->setValueOptions($genderList);
            $bloodGroupId->setValueOptions($bloodGroupList);
            $religionId->setValueOptions($religionList);
            $ethnicityId->setValueOptions($ethnicityList);
    ...
    public function editAction() {
            ...
            switch ($tab) {
                case 1:
                    ...
                    if ($this->formOne->isValid()) {
                        ...
                        if ($id == 0) {
                            ...
                            // echo ('<pre>');print_r($formOneModel);die;
                ...
                case 3:
                    ...
                    if ($this->formThree->isValid()) {
                        ...
                        $idCitizenIssueDtRaw = date("Y-m-d", strtotime($formThreeModel->idCitizenshipIssueDate));
                        ...
                        $birthDate = $this->repository->getBirthDate($id);
                        $ageValidation = $idCitizenIssueDtRaw - $birthDate['BIRTH_DATE'];
                        if (($birthDate['BIRTH_DATE'] > $idCitizenIssueDtRaw) || ($ageValidation < 16)){
                ...
                case 4:
                    ...
                    if ($this->formFour->isValid()) {
                        ...
                        $formFourModel->actingPositionId = $postData->actingPositionId;
                        // print_r($formFourModel);die;
                        $formFourModel->modifiedBy = $this->employeeId;
                        $formFourModel->modifiedDt = Helper::getcurrentExpressionDate();

                        $joinDtRaw = date("Y-m-d", strtotime($formFourModel->joinDate));
                        // $gratuityDtRaw = date("Y-m-d", strtotime($formFourModel->gratuityDate));
                        // $parmanentDtRaw = date("Y-m-d", strtotime($formFourModel->permanentDate));

                        $formFourModel->joinDate = Helper::getExpressionDate($formFourModel->joinDate);
                        $formFourModel->gratuityDate = Helper::getExpressionDate($formFourModel->gratuityDate);
                        $formFourModel->permanentDate = Helper::getExpressionDate($formFourModel->permanentDate);

                        $birthDate = $this->repository->getBirthDate($id);
                        $joinDateValidation = $joinDtRaw - $birthDate['BIRTH_DATE'];
                        // $gratuityDateValidation = $gratuityDtRaw - $birthDate['BIRTH_DATE'];
                        // $parmanentDateValidation = $parmanentDtRaw - $birthDate['BIRTH_DATE'];

                        if (($joinDateValidation < 16)){ // || ($gratuityDateValidation < 16) || ($parmanentDateValidation < 16
                            $this->flashmessenger()->addMessage(' Invalid Join Date.');     //or/and Gratuity Date or/and Permanent Date
                            return $this->redirect()->toRoute('employee', ['action' => 'edit', 'id' => $id, 'tab' => 4]);
                        }
                        // print_r($formFourModel);die;
                        ...
                        if($recommenderId){
                            ...
                            $recommendApprove->recommendBy = null;
                        }
                        if($approverId){
                            $recommendApprove->approvedBy = $approverId;
                        }else{
                            $recommendApprove->approvedBy = null;
                        }
                        ...
                        if($postData->update == 'Y'){
                            ...
                            //Mapping for job History start
                            $jobHistoryModel = new JobHistory();
                            $jobHistoryModel->jobHistoryId = Helper::getMaxId($this->adapter, JobHistory::TABLE_NAME, JobHistory::JOB_HISTORY_ID) + 1;
                            $jobHistoryModel->employeeId = $data->employeeId;

                            $startDtRaw = date("Y-m-d", strtotime($data->startDate));
                            $endDtRaw = date("Y-m-d", strtotime($data->endDate));
                            
                            $jobHistoryModel->startDate = Helper::getExpressionDate($data->startDate);
                            $jobHistoryModel->endDate = Helper::getExpressionDate($data->endDate);
                            $jobHistoryModel->serviceEventTypeId = $data->serviceEventTypeId;
                            $jobHistoryModel->branchId = $data->branchId;
                            $jobHistoryModel->departmentId = $data->departmentId;
                            $jobHistoryModel->designationId = $data->designationId;
                            $jobHistoryModel->positionId = $data->positionId;
                            $jobHistoryModel->serviceTypeId = $data->positionId;
                            $jobHistoryModel->createdBy = $data->createdBy;
                            $jobHistoryModel->salary = $data->salary;
                            $jobHistoryModel->status = 'E';
                            $jobHistoryModel->eventDate = Helper::getExpressionDate($data->eventDate);
                            //Mapping for job History end

                            $birthDate = $this->repository->getBirthDate($id);
                            $startDateValidation = $startDtRaw - $birthDate['BIRTH_DATE'];

                            // $endDateValidation = $endDtRaw - $startDtRaw;

                            // print_r($endDateValidation);die;

                            if (($startDateValidation < 16 || $startDtRaw > $endDtRaw)){
                                $this->flashmessenger()->addMessage(' Invalid Service Status Start/End Date.');
                                return $this->redirect()->toRoute('employee', ['action' => 'edit', 'id' => $id, 'tab' => 4]);
                            }


                            $this->repository->updateServiceStatus($jobHistoryModel);
                        }
        ...
        $client_name = $this->repository->getClientDetail()['CLIENT'];
        // print_r(ApplicationHelper::getTableKVListWithSortOption($this->adapter, "HRIS_SERVICES", "SERVICE_ID", ["SERVICE_NAME"], ["STATUS" => 'E'], "SERVICE_NAME", "ASC", null, true, true));die;
        return Helper::addFlashMessagesToArray($this, [
                ...
                'services' => ApplicationHelper::getTableKVListWithSortOption($this->adapter, "HRIS_SERVICES", "SERVICE_ID", ["SERVICE_NAME"], ["STATUS" => 'E'], "SERVICE_NAME", "ASC", null, true, true),
                'serviceSubgroup' => ApplicationHelper::getTableKVListWithSortOption($this->adapter, "HRIS_SERVICE_SUBGROUP", "SERVICE_SUBGROUP_ID", ["SERVICE_NAME"], ["STATUS" => 'E'], "SERVICE_NAME", "ASC", null, true, true),
                'classes' => ApplicationHelper::getTableKVListWithSortOption($this->adapter, "HRIS_CLASSES", "CLASS_ID", ["CLASS_NAME"], ["STATUS" => 'E'], "CLASS_ID", "ASC", null, true, true),
                ...
                'oscompanys' => ApplicationHelper::getTableKVListWithSortOption($this->adapter, "HRIS_OSCOMPANY", "COMPANY_ID", ["COMPANY_NAME"], ["STATUS" => 'E'], "COMPANY_NAME", "ASC", null, true, true),
                ...
                'fileNames' => ApplicationHelper::getTableKVList($this->adapter, 'HRIS_EMPLOYEE_FILE_SETUP', 'FILE_ID', ['FILE_NAME'],"Status='E'"),
        ...
        try {
            ...
            if ($experienceListEmpty == 1) {
                ...
                foreach ($experienceList as $experience) {
                    ...
                    $employeeExperienceModel->fromDate = Helper::getExpressionDate($experience['fromDate']);
                    $employeeExperienceModel->toDate = Helper::getExpressionDate($experience['toDate']);
                    $employeeExperienceModel->position = $experience['position'];
                    $employeeExperienceModel->address = $experience['address'];
        ...
        try {
            ...
            if ($trainingListEmpty == 1) {
                ...
                foreach ($trainingList as $training) {
                    ...
                    $employeeTrainingModel->fromDate = Helper::getExpressionDate($training['fromDate']);
                    $employeeTrainingModel->toDate = Helper::getExpressionDate($training['toDate']);
                    $employeeTrainingModel->location = $training['location'];
                    $employeeTrainingModel->deliveredBy = $training['deliveredBy'];
                    $employeeTrainingModel->funding = $training['funding'];
                    $employeeTrainingModel->certification = $training['certification'];
                    $employeeTrainingModel->totalDays = $training['toDate'] - $training['fromDate'] + 1;
    ...
    public function pushEmployeeProfileAction() {
        try {
            ...
            if ($data['fileCode'] == null) {
                ...
                $employeefile->filetypeCode = '';
                $employeefile->employeeId = null;   // Sending emp id as null
                $employeefile->filePath = $data['filePath'];
                // $employeefile->fileId = $data['fileName'];   // Old file id as file name.
                $employeefile->fileId = 0;
    ...
    public function pushEmployeeDocumentAction() {
        try {
            ...
            $employeefile->filetypeCode = $data['fileTypeCode'];
            $employeefile->filePath = $data['filePath'];
            $employeefile->fileId = $data['oldFileName'];
            $employeefile->status = 'E';
            $employeefile->createdDt = Helper::getcurrentExpressionDate();
    ...
    public function dropEmployeeFileAction() {
        try {
            ...
            // Unlink File
            if(file_exists(Helper::UPLOAD_DIR . '/'. $data['filePath'])){
                unlink(Helper::UPLOAD_DIR . '/'. $data['filePath']);
            }else{
        ...
        try {
            ...
            if ($relationListEmpty == 1) {
                ...
                foreach ($relationList as $relations) {
                    ...
                    $employeeRelationModel->dob=Helper::getcurrentExpressionDate($relations['dob']) ;
                    $employeeRelationModel->isDependent=$relations['isDependent']['id'] ;
                    $employeeRelationModel->isNominee=$relations['isNominee']['id'] ;
                    $employeeRelationModel->contactNo=$relations['contactNo'];
    ...
    public function deleteRelationDtlAction() {
        try {
            ...
            return new JsonModel(['success' => true, 'data' => "
             Detail Successfully Removed", 'message' => null]);
        } catch (Exception $e) {
            return new JsonModel(['success' => false, 'data' => null, 'message' => $e->getMessage()]);
        }
    }
...
//            } 
    ...
    public function checkVerifyAction(){
        $id = (int) $this->params()->fromRoute('id');
        $checkApprove = $this->params()->fromRoute('tab');
        $checkApproveIdentifier = preg_replace('/[^a-zA-Z]/', '', $checkApprove);
        $checkApproveStatus = preg_replace('/[^0-9]/', '', $checkApprove);
        if (0 === $id) {
            return $this->redirect()->toRoute('employee', ['action' => 'index']);
        }
        if($checkApproveIdentifier != 'C' && $checkApproveIdentifier != 'V' ) {
            return $this->redirect()->toRoute('employee', ['action' => 'index']);
        }

        $employee = new HrEmployees();
        $employee->employeeId = $id;
        if($checkApproveIdentifier == C){
            $employee->checked = $checkApproveStatus == 1 ? 'Y' : 'N';
            $employee->checkedBy = $this->employeeId;
            $employee->checkedDt = Helper::getcurrentExpressionDateTime();
        } else if ($checkApproveIdentifier == V){
            $employee->verified = $checkApproveStatus == 1 ? 'Y' : 'N';
            $employee->verifiedBy = $this->employeeId;
            $employee->verifiedDt = Helper::getcurrentExpressionDateTime();
        }
        $this->repository->edit($employee, $id);
        $this->flashmessenger()->addMessage("Employee Successfully Edited!!!");
        return $this->redirect()->toRoute('employee');
    }

    public function checkUserBankAccountAction() {
        try {
           
            $request = $this->getRequest();
            
            
            $id_account_no = $request->getPost('id_account_no');

            
            $employee_id = $request->getPost('employee_id');
            
            $id_account_no = trim($id_account_no);
            
            
            $returnData = $this->repository->checkUserBankAccountAvailability($id_account_no,$employee_id);
    ...
    public function checkUserLbrfAction() {
        try {
           
            $request = $this->getRequest();
            
            
            $id_lbrf =$request->getPost('id_lbrf');
            
            $employee_id = $request->getPost('employee_id');
            
            $id_lbrf = trim($id_lbrf);
            
            
            $returnData = $this->repository->checklbrfAvailability($id_lbrf,$employee_id);
