...
use Exception;
use Setup\Form\LocationNocForm;
use Setup\Model\LocationNoc;
use Setup\Repository\LocationNocRepository;
use Zend\Authentication\Storage\StorageInterface;
use Zend\Db\Adapter\AdapterInterface;
use Zend\View\Model\JsonModel;
use Zend\View\Model\ViewModel;

class LocationNocController extends HrisController {

    public function __construct(AdapterInterface $adapter, StorageInterface $storage) {
        parent::__construct($adapter, $storage);
        $this->initializeRepository(LocationNocRepository::class);
        $this->initializeForm(LocationNocForm::class);
    }
    ...
    public function editAction() {
        ...
        if ($id === 0) {
            return $this->redirect()->toRoute('locationNoc');
        }
        $this->prepareForm($id);
        $request = $this->getRequest();
        $location = new LocationNoc();
