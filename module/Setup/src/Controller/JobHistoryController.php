...
class JobHistoryController extends HrisController {
    ...
    public function addAction() {
        ...
        if ($request->isPost()) {
            ...
            if ($this->form->isValid()) {
                ...
                $empId = $jobHistory->employeeId;

                $startDtRaw = date("Y-m-d", strtotime($jobHistory->startDate));
                $endDtRaw = date("Y-m-d", strtotime($jobHistory->endDate));
                $eventDtRaw = date("Y-m-d", strtotime($jobHistory->eventDate));

                $joinDate = $this->repository->getJoinDate($empId);

                $startDateValidation = (strtotime($startDtRaw) - strtotime($joinDate['JOIN_DATE']))/86400;

                if (($startDateValidation <= 0)){ 
                    $this->flashmessenger()->addMessage(' Invalid Service Update Start Date.');
                    return $this->redirect()->toRoute('jobHistory', ['action' => 'add']);
                }

                //if ($eventDtRaw<=$startDtRaw || $eventDtRaw>=$endDtRaw){ 
                if (!$startDtRaw){
                    $this->flashmessenger()->addMessage(' Invalid Service Update Event Date.');
                    return $this->redirect()->toRoute('jobHistory', ['action' => 'add']);
                }
                ...
                $employeeDetails = $this->repository->getEmployeeDetails($empId);
                
                $jobHistory->fromBranchId = $employeeDetails['BRANCH_ID'];
                $jobHistory->fromDesignationId = $employeeDetails['DESIGNATION_ID'];
                $jobHistory->fromDepartmentId = $employeeDetails['DEPARTMENT_ID'];
                $jobHistory->fromPositionId = $employeeDetails['POSITION_ID'];
                $jobHistory->fromSeviceTypeId = $employeeDetails['SERVICE_TYPE_ID'];
                $jobHistory->fromCompanyId = $employeeDetails['COMPANY_ID'];
                $jobHistory->fromSalary = $employeeDetails['SALARY'];
                $jobHistory->fromFunctionalLevelId = $employeeDetails['FUNCTIONAL_LEVEL_ID'];
                $jobHistory->fromLocationId = $employeeDetails['LOCATION_ID'];
                $jobHistory->fromActingPositionId = $employeeDetails['ACTING_POSITION_ID'];
                $jobHistory->fromFunctionalLevelId= $employeeDetails['FUNCTIONAL_LEVEL_ID'];
                // echo('<pre>'); print_r($jobHistory);die;
        ...
        // print_r(EntityHelper::getTableKVListWithSortOption($this->adapter, "HRIS_POSITIONS", "POSITION_ID", ["POSITION_NAME"], ["STATUS" => 'E'], "POSITION_NAME", "ASC", null, true, true));die;
        return Helper::addFlashMessagesToArray($this, [
                    ...
                    'locations' => EntityHelper::getTableKVListWithSortOption($this->adapter, "HRIS_LOCATIONS", "LOCATION_ID", ["LOCATION_EDESC"], ["STATUS" => 'E'], "LOCATION_EDESC", "ASC", null, true, true),
                    ...
                    'functionalLevels' => EntityHelper::getTableKVListWithSortOption($this->adapter, "HRIS_FUNCTIONAL_LEVELS", "FUNCTIONAL_LEVEL_ID", ["FUNCTIONAL_LEVEL_EDESC"], ["STATUS" => 'E'], "FUNCTIONAL_LEVEL_EDESC", "ASC", null, true, true),
                    'services'  => EntityHelper::getTableKVListWithSortOption($this->adapter, "HRIS_SERVICES", "SERVICE_ID", ["SERVICE_NAME"], ["STATUS" => 'E'], "SERVICE_NAME", "ASC", null, true, true),
                    'serviceSubGroup' => EntityHelper::getTableKVListWithSortOption($this->adapter, "HRIS_SERVICE_SUBGROUP", "SERVICE_SUBGROUP_ID", ["SERVICE_NAME"], ["STATUS" => 'E'], "SERVICE_NAME", "ASC", null, true, true),
        ]);
    }
    ...
    public function editAction() {
        ...
        // echo '<pre>';  print_r ($jobHistoryDetail); die();
    ...
    public function getCurrentDetailsAction(){
        try {
            $request = $this->getRequest();
            if (!$request->isPost()) {
                throw new Exception("The request should be of type post");
            ...
            $result = $this->repository->fetchCurrentData($employeeId);
            return new CustomViewModel(['success' => true, 'data' => $result, 'error' => '']);
        } catch (Exception $e) {
            return new CustomViewModel(['success' => false, 'data' => [], 'error' => $e->getMessage()]);
        }
    ...
    public function printAction(){
        ...
        $printController = new PrintController($this->adapter);
        //print_r($jobHistory); die;
        $printData = $printController->printServiceEvent($jobHistory);
    ...
    public function getRolesDataAction(){
        try {
            $request = $this->getRequest();
            $data = $request->getPost();
            $result = $this->repository->fetchRolesByParameters($data['positionId'], $data['functionalLevelId']);
            return new CustomViewModel(['success' => true, 'data' => $result[0]['ROLES'], 'error' => '']);
        } catch (Exception $e) {
            return new CustomViewModel(['success' => false, 'data' => [], 'error' => $e->getMessage()]);
        }
    }

    public function getNewGradeAmountAction(){
        try {
            $request = $this->getRequest();
            if (!$request->isPost()) {
                throw new Exception("The request should be of type post");
            ...
            $toPositionIdVal = $data['toPositionIdVal'];
            $toFunctionalLevelIdVal = $data['toFunctionalLevelIdVal'];
            $toServiceTypeId = $data['toServiceTypeId'];
            $oldGradeAmount = $data['oldGradeAmount'];

            $result = $this->repository->fetchNewGradeAmount($toPositionIdVal,$toFunctionalLevelIdVal,$toServiceTypeId,$oldGradeAmount);
            if($result['newGradeSankhya']==0){
                return new CustomViewModel(['success' => false, 'data' => [], 'error' => 'Old grade amount is not detaermined or combination of postion, service type and functional level is not correct']);
            }else{
                return new CustomViewModel(['success' => true, 'data' => $result, 'error' => '']);
            }
        } catch (Exception $e) {
