...
class RecommendApproveRepository extends HrisRepository implements RepositoryInterface {
...
//    private $adapter;
    ...
    public function getEmployeeList($withinBranch, $withinDepartment, $withinLocation,$designationId, $branchId, $departmentId,$locationId) {
        $boundedParams = [];
        ...
        if ($withinLocation != null && $withinLocation != "N") {
        ...
        $boundedParams['locationId'] = $locationId;
        $statement = $this->adapter->query($sql);
    ...
    public function edit(Model $model, $id) {
        ...
        if(!$model->approvedBy){
          $array['APPROVED_BY'] = null;
        }
        if(!$model->recommendBy){
          $array['RECOMMEND_BY'] = null;
        }
    ...
    public function getFilteredList($search) {
        $condition = "";
        $condition .= EntityHelper::getSearchConditon($search['companyId'], $search['branchId'],$search['departmentId'], $search['positionId'], $search['designationId'], $search['serviceTypeId'], $search['serviceEventTypeId'], $search['employeeTypeId'], $search['employeeId'], null, $search['locationId'], $search['functionalTypeId']);
...
GROUP BY IARA.EMPLOYEE_ID) AA ON (AA.EMPLOYEE_ID=E.EMPLOYEE_ID)
    ...
    public function getEmployeeForOverride($data) {
        $boundedParams = [];
        $condition = EntityHelper::getSearchConditonBounded($data['companyId'], $data['branchId'],$data['locationId'], $data['departmentId'], $data['positionId'], $data['designationId'], $data['serviceTypeId'], $data['serviceEventTypeId'], $data['employeeTypeId'], $data['employeeId'], null, null, null);
        ...
        if ($data['type'] == 'LV' && ($data['typeId'] != null || $data['typeId'] != -1)) {
            $leaveTypeCondition = "AND TYPE_ID = ?";
    ...
    public function updateStatus($employeeId, $updateData) {
        ...
        if($updateData['type'] == 'LV') {
            $leaveTypeCheck = "AND TYPE_ID = ?";
            $boundedParams['leaveType'] = $updateData['leaveType'];
        }
        $updateSql = "UPDATE HRIS_REC_APP_OVERRIDE SET STATUS = 'D' where employee_id = ? and TYPE = ? " . $leaveTypeCheck;
        $this->rawQuery($updateSql, $boundedParams);
        return;
    }
