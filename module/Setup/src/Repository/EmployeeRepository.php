...
class EmployeeRepository extends HrisRepository implements RepositoryInterface {
    ...
    public function __construct(AdapterInterface $adapter, $tableName = null) {
        ...
        $this->jobHistoryGateway = new TableGateway('HRIS_JOB_HISTORY', $adapter);
    ...
    public function getById($id) {
        ...
        $select
                ...
                ->join(['ETH' => "HRIS_ETHNICITIES"], "E." . HrEmployees::ETHNICITY_ID . "=ETH.ETHNICITY_ID", ['ETHNICITY' => new Expression('INITCAP(ETH.ETHNICITY)')], 'left')
                ...
                ->join(['DI' => "HRIS_DISTRICTS"], "to_char(E." . HrEmployees::ID_CITIZENSHIP_ISSUE_PLACE . ")=to_char(DI.DISTRICT_ID)", ['CITIZENSHIP_ISSUE_PLACE' =>
                    ...
                    //     end')], 'left')
                  new Expression('case when 
                    ID_CITIZENSHIP_ISSUE_PLACE LIKE_REGEXPR \'^\d+(\.\d+)?$\'
                         then di.district_name
                         else ID_CITIZENSHIP_ISSUE_PLACE
                         end')], 'left')
                ...
                ->join(['P1' => Position::TABLE_NAME], "E." . HrEmployees::POSITION_ID . "=P1." . Position::POSITION_ID, ['POSITION_NAME' => new Expression('(P1.POSITION_NAME)'), "LEVEL_NO" => "LEVEL_NO"], 'left')
...
//        $this->tableGateway->update(['STATUS' => 'D', 'DELETED_DATE' => $model->deletedDate, 'DELETED_BY' => $model->deletedBy], ['EMPLOYEE_ID' => $model->employeeId]);
    ...
    public function fetchBy($by) {
        // $orderByString = EntityHelper::getOrderBy('E.FULL_NAME ASC', null, 'E.SENIORITY_LEVEL', 'P.LEVEL_NO', 'E.JOIN_DATE', 'DES.ORDER_NO', 'FULL_NAME');
        $columIfSynergy = "";
        $joinIfSyngery = "";
        ...
        $sql = "SELECT DISTINCT * FROM (SELECT 
            {$columIfSynergy}
                E.ID_ACCOUNT_NO  AS ID_ACCOUNT_NO,
                  ...
                  (VMP.VDC_MUNICIPALITY_NAME)                                AS VDC_MUNICIPALITY_NAME_PERM,
                  E.ADDR_TEMP_HOUSE_NO                                              AS ADDR_TEMP_HOUSE_NO,
                  E.ADDR_TEMP_WARD_NO                                               AS ADDR_TEMP_WARD_NO,
                  E.ADDR_TEMP_STREET_ADDRESS                                        AS ADDR_TEMP_STREET_ADDRESS,
                  ...
                  E.ID_DRIVING_LICENCE_NO                                           AS ID_DRIVING_LICENCE_NO,
                  E.ID_DRIVING_LICENCE_TYPE                                         AS ID_DRIVING_LICENCE_TYPE,
                  (TO_CHAR(E.ID_DRIVING_LICENCE_EXPIRY, 'DD-MON-YYYY'))      AS ID_DRIVING_LICENCE_EXPIRY,
                  E.ID_THUMB_ID                                                     AS ID_THUMB_ID,
                  E.ID_PAN_NO                                                       AS ID_PAN_NO,
                  E.ID_ACCOUNT_NO                                                   AS ID_ACCOUNT_NO,
                  E.ID_RETIREMENT_NO                                                AS ID_RETIREMENT_NO,
                  E.ID_CITIZENSHIP_NO                                               AS ID_CITIZENSHIP_NO,
                  (TO_CHAR(E.ID_CITIZENSHIP_ISSUE_DATE, 'DD-MON-YYYY'))      AS ID_CITIZENSHIP_ISSUE_DATE,
                  E.ID_CITIZENSHIP_ISSUE_PLACE                                      AS ID_CITIZENSHIP_ISSUE_PLACE,
                  E.ID_PASSPORT_NO                                                  AS ID_PASSPORT_NO,
                  (TO_CHAR(E.ID_PASSPORT_EXPIRY, 'DD-MON-YYYY'))             AS ID_PASSPORT_EXPIRY,
                  C.COMPANY_NAME                                                    AS COMPANY_NAME,
                  B.BRANCH_NAME                                                     AS BRANCH_NAME,
                  D.DEPARTMENT_NAME                                                 AS DEPARTMENT_NAME,
                  DES.DESIGNATION_TITLE                                             AS DESIGNATION_TITLE,
                  ...
                  E.VERIFIED,
                    (CASE 
                WHEN (E.FUNCTIONAL_LEVEL_ID IS NULL AND E.ACTING_FUNCTIONAL_LEVEL_ID IS NULL) THEN 100
                WHEN (E.FUNCTIONAL_LEVEL_ID = E.ACTING_FUNCTIONAL_LEVEL_ID OR E.ACTING_FUNCTIONAL_LEVEL_ID IS NULL)
                THEN FUNL.ORDER_ID
                ELSE AFUNL.ORDER_ID
                END) AS ORDER_ID,
                CASE WHEN ((SELECT EVENT_DATE FROM HRIS_JOB_HISTORY WHERE EMPLOYEE_ID = E.EMPLOYEE_ID AND JOB_HISTORY_ID 
                = (SELECT MAX(JOB_HISTORY_ID) FROM HRIS_JOB_HISTORY WHERE EMPLOYEE_ID = E.EMPLOYEE_ID
                AND SERVICE_EVENT_TYPE_ID IN 
                (SELECT SERVICE_EVENT_TYPE_ID FROM HRIS_SERVICE_EVENT_TYPES WHERE SERVICE_EVENT_TYPE_CODE IN ('PRO','APP')))) IS NULL)
                THEN CURRENT_DATE ELSE
                (SELECT EVENT_DATE FROM HRIS_JOB_HISTORY WHERE EMPLOYEE_ID = E.EMPLOYEE_ID AND JOB_HISTORY_ID 
                = (SELECT MAX(JOB_HISTORY_ID) FROM HRIS_JOB_HISTORY WHERE EMPLOYEE_ID = E.EMPLOYEE_ID
                AND SERVICE_EVENT_TYPE_ID IN 
                (SELECT SERVICE_EVENT_TYPE_ID FROM HRIS_SERVICE_EVENT_TYPES WHERE SERVICE_EVENT_TYPE_CODE IN ('PRO','APP')))) 
                END
                AS SERVICE_EVENT_DATE
                FROM HRIS_EMPLOYEES E
                LEFT JOIN HRIS_COMPANY C
                ON E.COMPANY_ID=C.COMPANY_ID
                LEFT JOIN HRIS_BRANCHES B
                ...
                LEFT JOIN HRIS_FUNCTIONAL_LEVELS FUNL
                ON E.FUNCTIONAL_LEVEL_ID=FUNL.FUNCTIONAL_LEVEL_ID
                LEFT JOIN HRIS_FUNCTIONAL_LEVELS AFUNL 
                ON E.ACTING_FUNCTIONAL_LEVEL_ID=AFUNL.FUNCTIONAL_LEVEL_ID
                LEFT JOIN HRIS_EMPLOYEE_FILE EF 
                ...
                {$condition['sql']})
                ORDER BY ORDER_ID, SERVICE_EVENT_DATE, FULL_NAME";
                // print_r($sql);die;
...
//        print_r($statement->getSql()); die();
    ...
    public function vdcStringToId($districtId, $vdc) {
        ...
        $sql = "
            ...
            WHERE DISTRICT_ID                      = ?
            AND LOWER(TRIM(VDC_MUNICIPALITY_NAME)) = LOWER(TRIM(?))";
   ...
   E.FAM_SPOUSE_NAME AS FAM_SPOUSE_NAME,
   E.FAM_SPOUSE_OCCUPATION AS FAM_SPOUSE_OCCUPATION,
   E.FAM_SPOUSE_BIRTH_DATE AS FAM_SPOUSE_BIRTH_DATE,
   E.FAM_SPOUSE_WEDDING_ANNIVERSARY AS FAM_SPOUSE_WEDDING_ANNIVERSARY,
   E.ID_CARD_NO AS ID_CARD_NO,
   E.ID_LBRF AS ID_LBRF,
   E.ID_BAR_CODE AS ID_BAR_CODE,
   E.ID_PROVIDENT_FUND_NO AS ID_PROVIDENT_FUND_NO,
   E.ID_DRIVING_LICENCE_NO AS ID_DRIVING_LICENCE_NO,
   E.ID_DRIVING_LICENCE_TYPE AS ID_DRIVING_LICENCE_TYPE,
  E.ID_DRIVING_LICENCE_EXPIRY AS ID_DRIVING_LICENCE_EXPIRY,
   E.ID_THUMB_ID AS ID_THUMB_ID,
   E.ID_PAN_NO AS ID_PAN_NO,
   E.ID_ACCOUNT_NO AS ID_ACCOUNT_NO,
   E.ID_RETIREMENT_NO AS ID_RETIREMENT_NO,
   E.ID_CITIZENSHIP_NO AS ID_CITIZENSHIP_NO,
   E.ID_CITIZENSHIP_ISSUE_DATE AS ID_CITIZENSHIP_ISSUE_DATE,
   E.ID_CITIZENSHIP_ISSUE_PLACE AS ID_CITIZENSHIP_ISSUE_PLACE,
   E.ID_PASSPORT_NO AS ID_PASSPORT_NO,
   E.ID_PASSPORT_EXPIRY AS ID_PASSPORT_EXPIRY,
...
LEFT JOIN HRIS_GENDERS G ON E.GENDER_ID=G.GENDER_ID 
LEFT JOIN HRIS_BLOOD_GROUPS BG ON E.BLOOD_GROUP_ID=BG.BLOOD_GROUP_ID 
LEFT JOIN HRIS_RELIGIONS RG ON E.RELIGION_ID=RG.RELIGION_ID 
LEFT JOIN HRIS_COUNTRIES CN ON E.COUNTRY_ID=CN.COUNTRY_ID 
LEFT JOIN HRIS_VDC_MUNICIPALITIES VM ON E.ADDR_PERM_VDC_MUNICIPALITY_ID=VM.VDC_MUNICIPALITY_ID 
LEFT JOIN HRIS_VDC_MUNICIPALITIES VM1 ON E.ADDR_TEMP_VDC_MUNICIPALITY_ID=VM1.VDC_MUNICIPALITY_ID 
LEFT JOIN HRIS_DEPARTMENTS D1 ON E.DEPARTMENT_ID=D1.DEPARTMENT_ID 
LEFT JOIN HRIS_DESIGNATIONS DES1 ON E.DESIGNATION_ID=DES1.DESIGNATION_ID 
LEFT JOIN HRIS_POSITIONS P1 ON E.POSITION_ID=P1.POSITION_ID 
LEFT JOIN HRIS_SERVICE_TYPES S1 ON E.SERVICE_TYPE_ID=S1.SERVICE_TYPE_ID 
LEFT JOIN HRIS_SERVICE_EVENT_TYPES SE1 ON E.SERVICE_EVENT_TYPE_ID=SE1.SERVICE_EVENT_TYPE_ID 
LEFT JOIN HRIS_EMPLOYEE_FILE EF ON E.PROFILE_PICTURE_ID = EF.FILE_CODE 
LEFT JOIN HRIS_EMPLOYEE_FILE CF ON C.LOGO =CF.FILE_CODE 
...
";
    ...
    public function setupEmployee($id) {
        $boundedParams = [];
        $sql = "CALL HRIS_EMPLOYEE_SETUP_PROC(?);";
        $boundedParams['id'] = $id;
        $statement = $this->adapter->query($sql);
    ...
    public function updateJobHistory($employeeId) {
        $boundedParams = [];
        $sql = "CALL HRIS_UPDATE_JOB_HISTORY(?);";
        $boundedParams['employeeId'] = $employeeId;
        $statement = $this->adapter->query($sql);
        $statement->execute($boundedParams);
    }

    public function updateServiceStatus($model) {
        $this->jobHistoryGateway->insert($model->getArrayCopyForDB());
    }
    ...
    public function getCompanyCodeByEmpId($employeeId) {
        $boundedParams = [];
        $sql = "SELECT ifnull(C.COMPANY_CODE,TO_CHAR(C.COMPANY_ID)) AS COMPANY_CODE FROM HRIS_EMPLOYEES E LEFT JOIN HRIS_COMPANY C ON (E.COMPANY_ID=C.COMPANY_ID) "
                . "WHERE E.EMPLOYEE_ID= ?";
        $boundedParams['employeeId'] = $employeeId;
        $statement = $this->adapter->query($sql);
        $result = $statement->execute($boundedParams);
    ...
    public function filterRecordsWithAR($employeeId, $branchId, $departmentId, $designationId, $positionId, $serviceTypeId, $serviceEventTypeId, $getResult = null, $companyId = null, $employeeTypeId = null, $locationId = null) {
        $boundedParams = [];
        $condition = EntityHelper::getSearchConditonBounded($companyId, $branchId, $departmentId, $positionId, $designationId, $serviceTypeId, $serviceEventTypeId, $employeeTypeId, $employeeId, null, $locationId, null);
        $boundedParams = array_merge($boundedParams, $condition['parameter']);
        $sql = "SELECT 
...
E.EMPLOYEE_ID                                                AS EMPLOYEE_ID,
            ...
            SELECT 
IARA.EMPLOYEE_ID,
STRING_AGG(IARA.R_A_ID, ','ORDER BY IARAE.FULL_NAME) AS A_R_ID,
STRING_AGG(IARAE.FULL_NAME, ','ORDER BY IARAE.FULL_NAME) AS A_R_NAME
...
LEFT JOIN (
            SELECT 
IARA.EMPLOYEE_ID,
STRING_AGG(IARA.R_A_ID, ','ORDER BY IARAE.FULL_NAME) AS A_A_ID,
STRING_AGG(IARAE.FULL_NAME, ','ORDER BY IARAE.FULL_NAME) AS A_A_NAME
FROM HRIS_ALTERNATE_R_A IARA
  ...
  FROM (SELECT EXCEPTIONAL  FROM  HRIS_DEPARTMENTS WHERE DEPARTMENT_ID IN  (INVALUES))
   CONNECT BY  REGEXP_SUBSTR(EXCEPTIONAL, '[^,]+', 1, LEVEL) IS NOT NULL
    ...
    public function getClientDetail(){
        $sql = "SELECT value as CLIENT from HRIS_PREFERENCES where key = 'CLIENT_NAME' ";
        $statement = $this->adapter->query($sql);
        return $statement->execute()->current();;
    }

    public function getBirthDate($id){
        $sql = "SELECT BIRTH_DATE FROM HRIS_EMPLOYEES WHERE EMPLOYEE_ID = {$id}";
        $statement = $this->adapter->query($sql);
        return $statement->execute()->current();
    }

    public function isProfileVerified($employeeId) {
        $sql = "SELECT VERIFIED FROM HRIS_EMPLOYEES WHERE EMPLOYEE_ID = {$employeeId}";
        $statement = $this->adapter->query($sql);
        return $statement->execute()->current();
    }

    public function checkUserBankAccountAvailability($id_account_no,$employee_id) {
       
        $boundedParameter = [];
        $boundedParameter['id_account_no']=$id_account_no;
        $boundedParameter['employee_id']=$employee_id;
        $sql = "SELECT * FROM HRIS_EMPLOYEES WHERE id_account_no=? and employee_id  != ?  ";

        //echo 'bank_account_number='.$id_account_no;
    ...
    public function checkLbrfAvailability($id_lbrf,$employee_id) {
       
        $boundedParameter = [];
        $boundedParameter['id_lbrf']=$id_lbrf;
        $boundedParameter['employee_id']=$employee_id;
        $sql = "SELECT * FROM HRIS_EMPLOYEES WHERE id_lbrf=? and employee_id  != ?  ";
