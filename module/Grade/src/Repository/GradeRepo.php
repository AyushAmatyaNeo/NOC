...
//use Grade\Model\IncommingDocument;
...
class GradeRepo extends HrisRepository implements RepositoryInterface
{
    ...
    public function getEmployeeJoiningDates() {
        $sql = "select he.employee_id, he.join_date, bs_date(he.join_date) as join_date_bs, he.position_id, he.functional_level_id, hfl.functional_level_edesc  
        from hris_employees he
        inner join hris_functional_levels hfl
        on he.functional_level_id = hfl.functional_level_id and he.status='E' and he.retired_flag = 'N' and he.resigned_flag='N'";
    ...
    public function getGradeCeiling($position_type, $position_id, $functional_level_edesc) {
        $sql = "select grade_ceiling_no from hris_employees_grade_ceiling_master_setup where position_type = '{$position_type}' and position_id = {$position_id} and functional_level_edesc='{$functional_level_edesc}'";
    ...
    public function isGrageRokka($employee_id) {
        $sql = "select * from hris_job_history where employee_id={$employee_id} and service_event_type_id = (select service_event_type_id 
        from hris_service_event_types where service_event_type_code = 'GRR');";
    ...
    public function isGradeReward($employee_id) {
        $sql = "select * from hris_job_history where employee_id={$employee_id} and service_event_type_id = (select service_event_type_id 
        from hris_service_event_types where service_event_type_code = 'GRW');";

        $statement = $this->adapter->query($sql);
        $result = $statement->execute();
        //$final_arr = Helper::extractDbData($result);
        return Helper::extractDbData($result);
    }

    public function isEmployeePromoted($employee_id) {
        $sql = "select * from hris_job_history where employee_id={$employee_id} and service_event_type_id = (select service_event_type_id 
        from hris_service_event_types where service_event_type_code = 'PRO');";
    ...
    public function updateGradeValue($employee_id, $new_grade_sankhya, $modified_dt, $curr_fiscal_year) {
        $sql = "update hris_flat_value_detail set flat_value = {$new_grade_sankhya}, modified_dt='{$modified_dt}' where employee_id={$employee_id} and fiscal_year_id={$curr_fiscal_year} and flat_id = (select flat_id from hris_flat_value_setup where flat_code = 'GRDSAN');";
    ...
    public function getCode($date){
        // print_r ($date);die();
		$sql = "select reg_temp_code from dc_registration_draft where reg_draft_id  = (select max(reg_draft_id) from dc_registration_draft)";
    ...
    {
        $sql = "select dc.reg_temp_code,bs_date(dc.LETTER_REF_DATE) as LETTER_REF_MITI, dc.reg_draft_id,
       o.office_edesc,
       e.full_name,
