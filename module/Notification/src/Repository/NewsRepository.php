...
class NewsRepository extends HrisRepository implements RepositoryInterface {
    ...
    public function edit(Model $model, $id) {
        ...
        //unset empty data
        if(empty($data['NEWS_LDESC'])) {
          unset($data['NEWS_LDESC']);
        }
    ...
    public function allNewsTypeWise($typeId, $employeeId) {
        /*$sql = "SELECT AA.*,
              ...
              LEFT JOIN
                (SELECT NEWS_ID,
                STRING_AGG(FILE_PATH, ',' ORDER BY FILE_PATH) AS FILE_PATH
                FROM HRIS_NEWS_FILE
                GROUP BY NEWS_ID
                ) BB
              ON (BB.NEWS_ID=AA.NEWS_ID)
              LEFT JOIN
                (SELECT NEWS_ID,
                STRING_AGG(FILE_NAME, ',' ORDER BY FILE_NAME) AS FILE_NAME
                FROM HRIS_NEWS_FILE
                GROUP BY NEWS_ID
                ) CC
              ON (CC.NEWS_ID     =AA.NEWS_ID)
              ...
              AND NEWS_EXPIRY_DT>=CURRENT_DATE
              ORDER BY NEWS_DATE DESC";
      ...
      AND N.NEWS_TYPE    ={$typeId}";
        ...
        return $this->rawQuery($sql1, $boundedParameter);        
    ...
    public function newsAssign(int $newsId, array $assignedTo) {
        ...
        EntityHelper::rawQueryResult($this->adapter, "CALL HRIS_NEWS_TO_PROC(?);",$boundedParameter);
    }
