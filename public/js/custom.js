...
//                        if (nepaliDatePickerExt.getDate(toDate).getTime() > nepaliDatePickerExt.getDate(fromDate).getTime()) {
    ...
    var successMessage = function (message, title) {
        ...
        if (message && (message.length > 0)) {
            // window.toastr.success(message[0], title);   old code
...
//             to create export pdf button
    ...
    var exportToPDFSmallFontSize = function ($table, col, fileName, pageSize, fn) {
        if (!checkForFileExt(fileName)) {
            fileName = fileName + ".pdf";
        }
        var colWidths = [];
    ...
    var populateSelect = function ($element, list, id, value, defaultMessage, defaultValue, selectedId, isMandatory, trigger = true) {
    ...
    // Dropzone for service status update :-- Start --:

    var setDropZoneServiceUpdate = function ($fileId, $dropZoneService, url) {
        var dropZoneService = $dropZoneService.dropzone({
            url: url,
            maxFiles: 1,
            acceptedFiles: 'image/*',
            autoProcessQueue: true,
            ...
            serverRequest(document.getFileDetailLink, {fileId: $fileId.val()}).then(function (response) {
                if (response.success) {
                    ...
                    if(baseurl == 'http://localhost'){
                        var $li = $('<li class="list-group-item">' + '<a target="_blank" href=' +  baseurl + '/hana-noc/neo-hris/public/uploads/' + response.data['FILE_IN_DIR_NAME'] + '>' + response.data['FILE_NAME'] +  '</a>'  + '</li>');
                    } else{
                        var $li = $('<li class="list-group-item">' + '<a target="_blank" href=' +  baseurl + '/public/uploads/' + response.data['FILE_IN_DIR_NAME'] + '>' + response.data['FILE_NAME'] +  '</a>'  + '</li>');
                    }
                    $ul.append($li);
                    $ul.insertBefore($dropZoneService);
                }
            });
        }
    };
    // Dropzone for service status update :-- END --:
...
//        $('.reset-field').change();
    ...
    return {
        ...
        setDropZoneServiceUpdate: setDropZoneServiceUpdate,
        ...
        exportToPDFSmallFontSize: exportToPDFSmallFontSize,
    };
