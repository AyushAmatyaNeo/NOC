...
//            };
            ...
            var employeeSearchAndPopulate = function () {
                var searchParams = {'COMPANY_ID': $company.val(), 'BRANCH_ID': $branch.val(), 'DEPARTMENT_ID': $department.val(), 'DESIGNATION_ID': $designation.val(), 'POSITION_ID': $position.val(), 'SERVICE_TYPE_ID': $serviceType.val()};
                if ($gender.length != 0) {
                ...
                for(let i = 0; i < acl['CONTROL'].length; i++){
                    ...
                    switch (acl['CONTROL'][i]) {
                        ...
                        case 'P':
                            ...
                            break;
                        case 'L':
                            $location.val((populateValues.length<1)?employeeDetail['LOCATION_ID']:populateValues);
