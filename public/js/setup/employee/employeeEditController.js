(function ($, app) {
    ...
    'use strict';
    $("#osCompany").hide();
    if($("#employeeType").val() == 'O'){ 
        $("#osCompany").show();$("#osCompany").attr("required","requied");
        $("#osCompanyHeader").show();
    } else{
        $("#osCompany").hide().prop('required',false); 
        $("#osCompany").prop('required',false);
        $("#osCompanyHeader").hide();}
    ...
    angular.module("hris", ['ui.bootstrap'])
        .controller('qualificationController', function ($scope, $uibModal, $log, $document, $window) {
                ...
                $scope.$apply(function () {
                    ...
                    if (num == 0) {
                        ...
                        for (var j = 0; j < num; j++) {
                            ...
                            } else if (data.employeeQualificationList[j].rankType == 'DIV') {
                                var rankType = $scope.rankType[2];
                            }
            ...
            $scope.fileNames = document.fileNames;
            $scope.file = {
                fileCode: null,
                fileTypeCode: null,
                filePath: null,
            ...
            $scope.drop = function (fileCode,filePath, key) {
            ...
            $scope.addDocument = function() {
                var modalInstance = $uibModal.open({
                    ...
                    controller: function ($scope, $uibModalInstance, fileTypes, fileNames) {
                        $scope.ok = function () {
                            if (document.myDropzone.files.length == 0) {
                                $scope.valid = false;
                                return; 
                            };
                            if(typeof $scope.selectedFileTypes == 'undefined'){
                                $scope.selectedFileTypes = 'jpeg';
                            }                                         
                            var dropzoneFileType = document.myDropzone.files[0].type;
                            switch (dropzoneFileType) {
                                case 'image/jpeg':
                                    var typefile = 'jpeg';
                                    break;
                                ...
                                case 'application/pdf':
                                    var typefile = 'pdf';
                                    // var typefile = dropzoneFileType.substr(dropzoneFileType.length - 3);
                                    break;
                                case 'image/jpg':
                                    var typefile = 'jpg';
                                    break;
                            ...
                            $scope.dzfile = angular.uppercase(typefile);
                            var choosenFileType = angular.lowercase($scope.selectedFileTypes);
                            if(choosenFileType != typefile ){
                                $scope.filevalid = false;
                                return;
                            }
                            document.fileTypeCode = $scope.fileTypeCode;
                            document.fileNameId = $scope.fileNameId;
                        ...
                        $scope.fileTypeChange = function () {
                            $scope.selectedFileTypes = userFileType();
                        };
                        $scope.valid = true;
                        ...
                        $scope.fileNameId = Object.keys($scope.fileNames)[0];
                    }
                });
               var userFileType = function(){
                    var e = document.getElementById("file-type");
                ...
                modalInstance.rendered.then(function () {                    
                    var MyDropZoneOptions = {
                        ...
                        // acceptedFiles: '.'+selectedType,
                        autoProcessQueue: false,
                        maxFiles: 1,
                        addRemoveLinks: true
                        };            
                    document.myDropzone = new Dropzone("#dropZoneContainer", MyDropZoneOptions);
                    ...
                    document.myDropzone.on("success", function (file, success) {
                        $scope.$apply(function () {
                            ...
                            if (document.fileNameId != '') {
                                toInsertFileName = document.fileNameId;
                            }                            
                            modalInstance.close({
                                fileTypeCode: document.fileTypeCode,
                                fileName: uploadResponse.fileName,
                                // fileName: document.fileNameId,
                        ...
                        $scope.$apply(function () {
                            if (success.data != null) {
                                ...
                                $scope.employeeDocuments.push({
                                    ...
                                    FILE_NAME: $scope.fileNames[selectedItem.oldFileName],
                        ...
                        if (experienceNum > 0) {
                            ...
                            for (var j = 0; j < experienceNum; j++) {
                                ...
                                $scope.experienceFormList.push(angular.copy({
                                    ...
                                    address: experienceList[j].ADDRESS,
                        ...
                        if (trainingNum > 0) {
                            ...
                            for (var j = 0; j < trainingNum; j++) {
                                $scope.trainingFormList.push(angular.copy({
                                    ...
                                    location: trainingList[j].LOCATION,
                                    deliveredBy: trainingList[j].DELIVERED_BY,
                                    funding: parseFloat(trainingList[j].FUNDING),
                                    certification: trainingList[j].CERTIFICATION,
                        ...
                        if (relationNum > 0) {
                            ...
                            for (var j = 0; j < relationNum; j++) {
                                ...
                                $scope.relationFormList.push(angular.copy({
                                    ...
                                    contactNo: relationList[j].CONTACT_NO,
