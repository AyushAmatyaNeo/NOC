    ...
    $(document).ready(function () {
        ...
        var imageUpload = function (data) {
            $('#fileDetailsTbl').append('<tr>'
                    + '<input value="' + data.oldFileName + '"><td>' + data.oldFileName + '</td>'
        ...
        app.pullDataById(document.pullFilesbyId, {
                'id': document.id
            }).then(function (success) {
                if (success.success) {
                    for(let i = 0; i < success.data.length; i++){
                        $('#fileDetailsTbl').append('<tr><td>' + success.data[i].FILE_IN_DIR_NAME+ '</td>'
                            + '<td><a target="_blank" href="' + document.basePath + '/uploads/loan_files/' + success.data[i].FILE_NAME + '"><i class="fa fa-download"></i></a></td>'
                        + '<td><button type="button" class="btn btn-danger deleteFile">DELETE</button></td><td style="visibility:hidden;">' + success.data[i].FILE_NAME + '</td></tr>');
                    }
                }
            }, function (failure) {
        });
        ...
        let pay_codes = document.pay_codes;
