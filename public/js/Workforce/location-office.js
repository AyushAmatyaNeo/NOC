(function($,app){
    $(document).ready(function () {
        ...
        app.populateSelect($('.location'), document.locationList, 'LOCATION_ID', 'LOCATION_EDESC', '-select-',null, 1, true, false);
        ...
        app.populateSelect($('.serviceGroup'), document.serviceGroupList, 'SERVICE_ID', 'SERVICE_NAME', '-----',null, 1, false, false);
        ...
        var currentDetails = {};
        var locationChangeAction = function(){
            var locationId = $('#locationId').val();
            app.serverRequest(document.getWorkForceData, {locationId: locationId}).then(function (response) {
                if (response.success) {
    ...
    $('.deatilAddBtn').on('click', function () {
        ...
        app.populateSelect($('#workforceTable tbody').find('.serviceGroup:last'),document.serviceGroupList,'SERVICE_ID','SERVICE_NAME', '-----',null,1,false);
        });
    ...
    var quotaAction = function(){
        var locationId = $('#locationId').val();
        app.serverRequest(document.getQuotaData, {locationId: locationId}).then(function (response) {
            if (response.success) {
    ...
    function addPreFilledRows(item){
      ...
      app.populateSelect($('#workforceTable tbody').find('.designation:last'), document.designationList, 'DESIGNATION_ID', 'DESIGNATION_TITLE', '-----',null, 1, false);     
      app.populateSelect($('#workforceTable tbody').find('.serviceGroup:last'),document.serviceGroupList,'SERVICE_ID','SERVICE_NAME', '-----',null,1,false);
      app.populateSelect($('#workforceTable tbody').find('.serviceSubgroup:last'),document.getServiceSubgroupList[item.SERVICE_ID],'SERVICE_SUBGROUP_ID','SERVICE_NAME', '-----',null,1,false);       
